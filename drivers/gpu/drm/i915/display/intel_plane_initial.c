			...
			struct intel_initial_plane_config *plane_config)
{
	...
	if (dma_addr < mem->region.start || dma_addr > mem->region.end) {
		drm_err(display->drm,
			...
			&dma_addr, mem->region.name, &mem->region.start, &mem->region.end);
#elif defined(__FreeBSD__)
			"Initial plane programming using invalid range, dma_addr=%pa [%pa-%pa]\n",
			&dma_addr, &mem->region.start, &mem->region.end);
#endif
		return false;
	}
		  ...
		  struct intel_initial_plane_config *plane_config)
{
	...
	if (IS_ERR(obj)) {
		...
		drm_dbg_kms(display->drm, "Failed to preallocate initial FB in %s\n",
			    mem->region.name);
#elif defined(__FreeBSD__)
		drm_dbg_kms(display->drm, "Failed to preallocate initial FB\n");
#endif				
		return NULL;
	}
