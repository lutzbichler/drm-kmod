...
#ifdef __FreeBSD__
	ret = 0;
	dev_priv->gmch.mch_res_rid = 0x100;
	dev_priv->gmch.mch_res_bsd_res = bsd_intel_pci_bus_alloc_mem(
	    dev_priv->drm.dev->bsddev, &dev_priv->gmch.mch_res_rid, MCHBAR_SIZE,
	    &dev_priv->gmch.mch_res.start, &dev_priv->gmch.mch_res.end);
	if (dev_priv->gmch.mch_res_bsd_res == NULL)
		ret = -ENOMEM;
#else
...
void intel_gmch_bar_teardown(struct drm_i915_private *dev_priv)
{
	...
	if (dev_priv->gmch.mch_res_bsd_res != NULL) {
		bsd_intel_pci_bus_release_mem(dev_priv->drm.dev->bsddev,
		    dev_priv->gmch.mch_res_rid, dev_priv->gmch.mch_res_bsd_res);
		dev_priv->gmch.mch_res_bsd_res = NULL;
	}
#else
