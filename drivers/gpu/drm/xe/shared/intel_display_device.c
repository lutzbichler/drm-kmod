...
probe_gmdid_display(struct xe_device *xe, struct intel_display_ip_ver *ip_ver)
{
	...
	if (!addr) {
		drm_err(&xe->drm, "Cannot map MMIO BAR to read display GMD_ID\n");
		return NULL;
	}
	...
	if (val == 0) {
		drm_dbg_kms(&xe->drm, "Device doesn't have display\n");
		return NULL;
	}
	...
	drm_err(&xe->drm, "Unrecognized display IP version %d.%02d; disabling display.\n",
...
void intel_display_device_probe(struct xe_device *xe)
{
	...
	xe->display.drm = &xe->drm;

	intel_display_params_copy(&xe->display.params);
	...
	if (!desc) {
		drm_dbg_kms(&xe->drm, "Unknown device ID %04x; disabling display.\n",
			    pdev->device);
		goto no_display;
	}
	...
	info = desc->info;
	if (!info)
		info = probe_gmdid_display(xe, &ip_ver);
	if (!info)
		goto no_display;

	DISPLAY_INFO(xe) = info;

	memcpy(DISPLAY_RUNTIME_INFO(xe),
	       &DISPLAY_INFO(xe)->__runtime_defaults,
	       sizeof(*DISPLAY_RUNTIME_INFO(xe)));

	drm_WARN_ON(&xe->drm, !desc->platform || !desc->name);
	DISPLAY_RUNTIME_INFO(xe)->platform = desc->platform;

	subdesc = find_subplatform_desc(pdev, desc);
	if (subdesc) {
		drm_WARN_ON(&xe->drm, !subdesc->subplatform || !subdesc->name);
		DISPLAY_RUNTIME_INFO(xe)->subplatform = subdesc->subplatform;
	}

	if (ip_ver.ver || ip_ver.rel || ip_ver.step) {
		DISPLAY_RUNTIME_INFO(xe)->ip = ip_ver;
	...
	DISPLAY_RUNTIME_INFO(xe)->step = step;

	drm_info(&xe->drm, "Found %s%s%s (device ID %04x) display version %u.%02u stepping %s\n",
		 desc->name, subdesc ? "/" : "", subdesc ? subdesc->name : "",
		 pdev->device, DISPLAY_RUNTIME_INFO(xe)->ip.ver,
...
no_display:
	DISPLAY_INFO(xe) = &no_display;
}

void intel_display_device_remove(struct xe_device *xe)
{
	intel_display_params_free(&xe->display.params);
}

static void __intel_display_device_info_runtime_init(struct xe_device *xe)
{
	struct intel_display_runtime_info *display_runtime = DISPLAY_RUNTIME_INFO(xe);
	enum pipe pipe;
	...
	if (IS_HASWELL_ULT(xe) || IS_BROADWELL_ULT(xe))
		display_runtime->port_mask &= ~BIT(PORT_D);

	if (IS_ICL_WITH_PORT_F(xe))
		display_runtime->port_mask |= BIT(PORT_F);

	/* Wa_14011765242: adl-s A0,A1 */
	if (IS_ALDERLAKE_S(xe) && IS_DISPLAY_STEP(xe, STEP_A0, STEP_A2))
	...
	} else if (DISPLAY_VER(xe) >= 5 || IS_G4X(xe)) {
		for_each_pipe(xe, pipe)
			display_runtime->num_sprites[pipe] = 1;
	}

	if ((IS_DGFX(xe) || DISPLAY_VER(xe) >= 14) &&
	    !(intel_de_read(xe, GU_CNTL_PROTECTED) & DEPRESENT)) {
		drm_info(&xe->drm, "Display not present, disabling\n");
		goto display_fused_off;
	}

	if (IS_DISPLAY_VER(xe, 7, 8) && HAS_PCH_SPLIT(xe)) {
		u32 fuse_strap = intel_de_read(xe, FUSE_STRAP);
		u32 sfuse_strap = intel_de_read(xe, SFUSE_STRAP);
		...
		if (fuse_strap & ILK_INTERNAL_DISPLAY_DISABLE ||
			...
			drm_info(&xe->drm, "PipeC fused off\n");
			display_runtime->pipe_mask &= ~BIT(PIPE_C);
			display_runtime->cpu_transcoder_mask &= ~BIT(TRANSCODER_C);
		}
	} else if (DISPLAY_VER(xe) >= 9) {
		u32 dfsm = intel_de_read(xe, SKL_DFSM);
		...
		if (DISPLAY_VER(xe) >= 11 && (dfsm & ICL_DFSM_DMC_DISABLE))
			display_runtime->has_dmc = 0;

		if (IS_DISPLAY_VER(xe, 10, 12) &&
		    (dfsm & GLK_DFSM_DISPLAY_DSC_DISABLE))
			display_runtime->has_dsc = 0;
	}

	if (DISPLAY_VER(xe) >= 20) {
		u32 cap = intel_de_read(xe, XE2LPD_DE_CAP);
	...
	display_runtime->rawclk_freq = intel_read_rawclk(xe);
	drm_dbg_kms(&xe->drm, "rawclk rate: %d kHz\n", display_runtime->rawclk_freq);
...
void intel_display_device_info_runtime_init(struct xe_device *xe)
{
	if (HAS_DISPLAY(xe))
		__intel_display_device_info_runtime_init(xe);

	/* Display may have been disabled by runtime init */
	if (!HAS_DISPLAY(xe)) {
		xe->drm.driver_features &= ~(DRIVER_MODESET | DRIVER_ATOMIC);
		xe->display.info.__device_info = &no_display;
	}

	/* Disable nuclear pageflip by default on pre-g4x */
	if (!xe->display.params.nuclear_pageflip &&
	    DISPLAY_VER(xe) < 5 && !IS_G4X(xe))
		xe->drm.driver_features &= ~DRIVER_ATOMIC;
}
 ...
 */
bool intel_display_device_enabled(struct xe_device *xe)
{
	struct intel_display *display = &xe->display;
