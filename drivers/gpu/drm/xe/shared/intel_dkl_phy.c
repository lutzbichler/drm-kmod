...
static void
dkl_phy_set_hip_idx(struct xe_device *xe, struct intel_dkl_phy_reg reg)
{
	enum tc_port tc_port = DKL_REG_TC_PORT(reg);

	drm_WARN_ON(&xe->drm, tc_port < TC_PORT_1 || tc_port >= I915_MAX_TC_PORTS);
...
intel_dkl_phy_read(struct xe_device *xe, struct intel_dkl_phy_reg reg)
{
	...
	spin_lock(&xe->display.dkl.phy_lock);

	dkl_phy_set_hip_idx(xe, reg);
	val = intel_de_read(xe, DKL_REG_MMIO(reg));

	spin_unlock(&xe->display.dkl.phy_lock);
...
intel_dkl_phy_write(struct xe_device *xe, struct intel_dkl_phy_reg reg, u32 val)
{
	...
	dkl_phy_set_hip_idx(xe, reg);
	intel_de_write(xe, DKL_REG_MMIO(reg), val);
...
intel_dkl_phy_rmw(struct xe_device *xe, struct intel_dkl_phy_reg reg, u32 clear, u32 set)
{
	...
	dkl_phy_set_hip_idx(xe, reg);
	intel_de_rmw(xe, DKL_REG_MMIO(reg), clear, set);
...
intel_dkl_phy_posting_read(struct xe_device *xe, struct intel_dkl_phy_reg reg)
{
	...
	dkl_phy_set_hip_idx(xe, reg);
	intel_de_posting_read(xe, DKL_REG_MMIO(reg));
