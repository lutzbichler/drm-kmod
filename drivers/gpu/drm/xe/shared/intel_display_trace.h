...
trace_intel_pipe_enable(struct intel_crtc *crtc)
{
	...
	for_each_intel_crtc(&xe->drm, it__) {
...
static inline void
trace_intel_cpu_fifo_underrun(struct xe_device *xe, enum pipe pipe)
{
#ifdef KTR
...
static inline void
trace_intel_pch_fifo_underrun(struct xe_device *xe, enum pipe pch_transcoder)
{
#ifdef KTR
	...
	CTR4(KTR_DRM,
	    ...
	    __dev_name_xe(xe), pipe_name(pipe),
	    intel_crtc_get_vblank_counter(crtc), intel_get_crtc_scanline(crtc));
}

static inline void
trace_intel_memory_cxsr(struct xe_device *xe, bool old, bool new)
{
#ifdef KTR
	...
	CTR3(KTR_DRM,
	    ...
	    __dev_name_xe(xe), str_on_off(old), str_on_off(new));
	CTR6(KTR_DRM, 
...
{
#ifdef KTR
	struct intel_crtc *crtc = intel_crtc_for_pipe(to_xe(plane->base.dev),
	    plane->pipe);
#endif
...
static inline void
trace_intel_frontbuffer_invalidate(struct xe_device *xe,
    unsigned int frontbuffer_bits, unsigned int origin)
{
	CTR3(KTR_DRM,
	    "intel_frontbuffer_invalidate: dev %s, frontbuffer_bits=0x%08x, origin=%u",
	    __dev_name_xe(xe), frontbuffer_bits, origin);
}

static inline void
trace_intel_frontbuffer_flush(struct xe_device *xe,
    unsigned int frontbuffer_bits, unsigned int origin)
{
	CTR3(KTR_DRM,
