...
#define check_array_bounds(xe, a, i) drm_WARN_ON(&(xe)->drm, (i) >= ARRAY_SIZE(a))
...
static bool plane_has_modifier(struct xe_device *xe,
			       u8 plane_caps,
			       const struct intel_modifier_desc *md)
{
	if (!IS_DISPLAY_VER(xe, md->display_ver.from, md->display_ver.until))
		return false;
	...
	if (intel_fb_is_ccs_modifier(md->modifier) &&
	    HAS_FLAT_CCS(xe) != !md->ccs.packed_aux_planes)
		return false;

	if (md->modifier == I915_FORMAT_MOD_4_TILED_BMG_CCS &&
	    (GRAPHICS_VER(xe) < 20 || !IS_DGFX(xe)))
		return false;
 ...
 *
 * Returns:
 * Returns the list of modifiers allowed by the @xe platform and @plane_caps.
 * The caller must free the returned buffer.
 */
u64 *intel_fb_plane_get_modifiers(struct xe_device *xe,
				  u8 plane_caps)
{
	u64 *list, *p;
	...
	for (i = 0; i < ARRAY_SIZE(intel_modifiers); i++) {
		if (plane_has_modifier(xe, plane_caps, &intel_modifiers[i]))
			count++;
	}

	list = kmalloc_array(count, sizeof(*list), GFP_KERNEL);
	if (drm_WARN_ON(&xe->drm, !list))
		return NULL;
...
{
	const struct intel_modifier_desc *md = lookup_modifier(fb->modifier);
	struct xe_device *xe = to_xe(fb->dev);
...
unsigned int intel_tile_size(const struct xe_device *xe)
{
	return DISPLAY_VER(xe) == 2 ? 2048 : 4096;
}
...
intel_tile_width_bytes(const struct drm_framebuffer *fb, int color_plane)
{
	...
	case I915_FORMAT_MOD_Y_TILED:
		if (DISPLAY_VER(xe) == 2 || HAS_128_BYTE_Y_TILING(xe))
			return 128;
		else
			return 512;
...
{
	return intel_tile_size(to_xe(fb->dev)) /
...
bool intel_fb_modifier_uses_dpt(struct xe_device *xe, u64 modifier)
{
	return HAS_DPT(xe) && modifier != DRM_FORMAT_MOD_LINEAR;
}

bool intel_fb_uses_dpt(const struct drm_framebuffer *fb)
{
	return to_xe(fb->dev)->display.params.enable_dpt &&
		intel_fb_modifier_uses_dpt(to_xe(fb->dev), fb->modifier);
}
				       ...
				       u32 old_offset, u32 new_offset)
{
	...
	drm_WARN_ON(&xe->drm, new_offset > old_offset);

	if (!is_surface_linear(fb, color_plane)) {
		unsigned int tile_size, tile_width, tile_height;
		unsigned int pitch_tiles;

		tile_size = intel_tile_size(xe);
		intel_tile_dims(fb, color_plane, &tile_width, &tile_height);
 ...
 */
static u32 intel_compute_aligned_offset(struct xe_device *xe,
					int *x, int *y,
					const struct drm_framebuffer *fb,
					int color_plane,
					unsigned int pitch,
				       ...
				       int color_plane)
{
	...
	return intel_compute_aligned_offset(xe, x, y, fb, color_plane,
...
static bool intel_plane_can_remap(const struct intel_plane_state *plane_state)
{
	...
	if (fb->modifier == DRM_FORMAT_MOD_LINEAR) {
		unsigned int alignment = intel_tile_size(xe) - 1;
...
{
	struct xe_device *xe = to_xe(fb->base.dev);

	return (IS_ALDERLAKE_P(xe) || DISPLAY_VER(xe) >= 14) &&
...
{
	struct xe_bo *obj = intel_fb_obj(&fb->base);
	int ret;
...
{
	struct xe_device *xe = to_xe(fb->base.dev);
	unsigned int tile_size = intel_tile_size(xe);
	u32 offset;

	offset = intel_compute_aligned_offset(xe, x, y, &fb->base, color_plane,
			...
			int x, int y)
{
	...
	return DIV_ROUND_UP(size, intel_tile_size(xe));
}

#define assign_chk_ovf(xe, var, val) ({ \
	drm_WARN_ON(&(xe)->drm, overflows_type(val, var)); \
	(var) = (val); \
})

#define assign_bfld_chk_ovf(xe, var, val) ({ \
	(var) = (val); \
	drm_WARN_ON(&(xe)->drm, (var) != (val)); \
	(var); \
})
				 ...
				 struct intel_fb_view *view)
{
	...
	assign_bfld_chk_ovf(xe, remap_info->offset, obj_offset);

	if (intel_fb_is_gen12_ccs_aux_plane(&fb->base, color_plane)) {
		remap_info->linear = 1;

		assign_chk_ovf(xe, remap_info->size,
			       plane_view_linear_tiles(fb, color_plane, dims, x, y));
	} else {
		remap_info->linear = 0;

		assign_chk_ovf(xe, remap_info->src_stride,
			       plane_view_src_stride_tiles(fb, color_plane, dims));
		assign_chk_ovf(xe, remap_info->width,
			       plane_view_width_tiles(fb, color_plane, dims, x));
		assign_chk_ovf(xe, remap_info->height,
			       plane_view_height_tiles(fb, color_plane, dims, y));
	}

	if (view->gtt.type == I915_GTT_VIEW_ROTATED) {
		drm_WARN_ON(&xe->drm, remap_info->linear);
		check_array_bounds(xe, view->gtt.rotated.plane, color_plane);

		assign_chk_ovf(xe, remap_info->dst_stride,
		...
		drm_WARN_ON(&xe->drm, view->gtt.type != I915_GTT_VIEW_REMAPPED);

		check_array_bounds(xe, view->gtt.remapped.plane, color_plane);
		...
		if (remap_info->linear) {
			...
			assign_chk_ovf(xe, remap_info->dst_stride, dst_stride);
...
static void intel_fb_view_init(struct xe_device *xe, struct intel_fb_view *view,
			       enum i915_gtt_view_type view_type)
{
	...
	if (view_type == I915_GTT_VIEW_REMAPPED &&
	    (IS_ALDERLAKE_P(xe) || DISPLAY_VER(xe) >= 14))
		view->gtt.remapped.plane_alignment = SZ_2M / PAGE_SIZE;
}

bool intel_fb_supports_90_270_rotation(const struct intel_framebuffer *fb)
{
	if (DISPLAY_VER(to_xe(fb->base.dev)) >= 13)
		return false;
...
static unsigned int intel_fb_min_alignment(const struct drm_framebuffer *fb)
{
	...
	for_each_intel_plane(&xe->drm, plane) {
		...
		drm_WARN_ON(&xe->drm, plane_min_alignment &&
...
int intel_fill_fb_info(struct xe_device *xe, struct intel_framebuffer *fb)
{
	struct xe_bo *obj = intel_fb_obj(&fb->base);
	...
	intel_fb_view_init(xe, &fb->normal_view, I915_GTT_VIEW_NORMAL);
	...
	if (intel_fb_supports_90_270_rotation(fb))
		intel_fb_view_init(xe, &fb->rotated_view, I915_GTT_VIEW_ROTATED);
	if (intel_fb_needs_pot_stride_remap(fb))
		intel_fb_view_init(xe, &fb->remapped_view, I915_GTT_VIEW_REMAPPED);
...
{
	struct xe_device *xe =
		to_xe(plane_state->uapi.plane->dev);
	...
	intel_fb_view_init(xe, &plane_state->view,
	...
	drm_WARN_ON(&xe->drm, intel_fb_is_ccs_modifier(fb->modifier));
...
static
u32 intel_fb_max_stride(struct xe_device *xe,
			u32 pixel_format, u64 modifier)
{
	 ...
	 */
	if (DISPLAY_VER(xe) < 4 || intel_fb_is_ccs_modifier(modifier) ||
	    intel_fb_modifier_uses_dpt(xe, modifier))
		return intel_plane_fb_max_stride(xe, pixel_format, modifier);
...
intel_fb_stride_alignment(const struct drm_framebuffer *fb, int color_plane)
{
	...
	if (is_surface_linear(fb, color_plane)) {
		unsigned int max_stride = intel_plane_fb_max_stride(xe,
								    fb->format->format,
								    fb->modifier);
	...
	if (intel_fb_is_ccs_modifier(fb->modifier)) {
		 ...
		 */
		else if ((DISPLAY_VER(xe) == 9 || IS_GEMINILAKE(xe)) &&
...
static int intel_user_framebuffer_create_handle(struct drm_framebuffer *fb,
						...
						unsigned int *handle)
{
	struct xe_bo *obj = intel_fb_obj(fb);
	struct xe_device *xe = to_xe(intel_bo_to_drm_bo(obj)->dev);
			   ...
			   struct drm_mode_fb_cmd2 *mode_cmd)
{
	...
	ret = -EINVAL;
	if (!drm_any_plane_has_format(&xe->drm,
	...
	max_stride = intel_fb_max_stride(xe, mode_cmd->pixel_format,
					 mode_cmd->modifier[0]);
	...
	drm_helper_mode_fill_fb_struct(&xe->drm, fb, mode_cmd);

	for (i = 0; i < fb->format->num_planes; i++) {
		unsigned int stride_alignment;

		if (mode_cmd->handles[i] != mode_cmd->handles[0]) {
			drm_dbg_kms(&xe->drm, "bad plane %d handle\n",
				    i);
			goto err_frontbuffer_put;
		}
	...
	ret = intel_fill_fb_info(xe, intel_fb);
	if (ret)
		goto err_frontbuffer_put;
	...
	if (intel_fb_uses_dpt(fb)) {
		...
		if (IS_ERR(vm)) {
			drm_dbg_kms(&xe->drm, "failed to create DPT\n");
			ret = PTR_ERR(vm);
			goto err_frontbuffer_put;
		}
	...
	ret = drm_framebuffer_init(&xe->drm, fb, &intel_fb_funcs);
	if (ret) {
		drm_err(&xe->drm, "framebuffer init failed %d\n", ret);
		goto err_free_dpt;
	}
			      ...
			      const struct drm_mode_fb_cmd2 *user_mode_cmd)
{
	...
	obj = intel_fb_bo_lookup_valid_bo(xe, filp, &mode_cmd);
	if (IS_ERR(obj))
		return ERR_CAST(obj);
...
struct drm_framebuffer *
intel_framebuffer_create(struct xe_bo *obj,
			 struct drm_mode_fb_cmd2 *mode_cmd)
{
