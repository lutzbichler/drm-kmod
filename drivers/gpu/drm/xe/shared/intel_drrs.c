...
{
	if (HAS_DOUBLE_BUFFERED_M_N(xe))
		return true;

	return intel_cpu_transcoder_has_m2_n2(xe, cpu_transcoder);
}
				     ...
				     enum drrs_refresh_rate refresh_rate)
{
	...
	intel_de_rmw(xe, TRANSCONF(xe, cpu_transcoder),
				 ...
				 enum drrs_refresh_rate refresh_rate)
{
	...
	if (intel_cpu_transcoder_has_m2_n2(xe, crtc->drrs.cpu_transcoder))
...
static void intel_drrs_schedule_work(struct intel_crtc *crtc)
{
	...
	mod_delayed_work(xe->unordered_wq, &crtc->drrs.work, msecs_to_jiffies(1000));
}
...
static unsigned int intel_drrs_frontbuffer_bits(const struct intel_crtc_state *crtc_state)
{
	...
	for_each_intel_crtc_in_pipe_mask(&xe->drm, crtc,
...
static void intel_drrs_frontbuffer_update(struct xe_device *xe,
...
void intel_drrs_invalidate(struct xe_device *xe,
			   unsigned int frontbuffer_bits)
{
	intel_drrs_frontbuffer_update(xe, frontbuffer_bits, true);
}
...
void intel_drrs_flush(struct xe_device *xe,
		      unsigned int frontbuffer_bits)
{
	intel_drrs_frontbuffer_update(xe, frontbuffer_bits, false);
}
...
static int intel_drrs_debugfs_status_show(struct seq_file *m, void *unused)
{
	...
	seq_printf(m, "DRRS capable: %s\n",
		   str_yes_no(intel_cpu_transcoder_has_drrs(xe,
