...
int intel_crtc_scanline_offset(const struct intel_crtc_state *crtc_state)
{
	...
	if (DISPLAY_VER(display) == 2)
		return -1;
	else if (HAS_DDI(xe) && intel_crtc_has_type(crtc_state, INTEL_OUTPUT_HDMI))
		return 2;
	else
		return 1;
}
...
static void intel_vblank_section_enter(struct intel_display *display)
	__acquires(xe->uncore.lock)
{
	struct xe_device *xe = to_xe(display->drm);
	spin_lock(&xe->uncore.lock);
}

static void intel_vblank_section_exit(struct intel_display *display)
	__releases(xe->uncore.lock)
{
	struct xe_device *xe = to_xe(display->drm);
	spin_unlock(&xe->uncore.lock);
}
#else
static void intel_vblank_section_enter(struct intel_display *display)
{
				     ...
				     const struct drm_display_mode *mode)
{
	...
	bool use_scanline_counter = DISPLAY_VER(display) >= 5 ||
		IS_G4X(xe) || DISPLAY_VER(display) == 2 ||
			     ...
			     struct intel_vblank_evade_ctx *evade)
{
	...
	evade->need_vlv_dsi_wa = (IS_VALLEYVIEW(xe) || IS_CHERRYVIEW(xe)) &&
