...
int intel_pmdemand_init(struct xe_device *xe)
{
	struct intel_pmdemand_state *pmdemand_state;
	...
	intel_atomic_global_obj_init(xe, &xe->display.pmdemand.obj,
				     &pmdemand_state->base,
				     &intel_pmdemand_funcs);

	if (IS_DISPLAY_VER_STEP(xe, IP_VER(14, 0), STEP_A0, STEP_C0))
		/* Wa_14016740474 */
		intel_de_rmw(xe, XELPD_CHICKEN_DCPR_3, 0, DMD_RSP_TIMEOUT_DISABLE);

	return 0;
}

void intel_pmdemand_init_early(struct xe_device *xe)
{
	mutex_init(&xe->display.pmdemand.lock);
	init_waitqueue_head(&xe->display.pmdemand.waitqueue);
}

void
intel_pmdemand_update_phys_mask(struct xe_device *xe,
...
void
intel_pmdemand_update_port_clock(struct xe_device *xe,
				 struct intel_pmdemand_state *pmdemand_state,
...
static void
intel_pmdemand_update_max_ddiclk(struct xe_device *xe,
				 ...
				 struct intel_pmdemand_state *pmdemand_state)
{
	...
	for_each_new_intel_crtc_in_state(state, crtc, new_crtc_state, i)
		intel_pmdemand_update_port_clock(xe, pmdemand_state,
						 crtc->pipe,
...
static void
intel_pmdemand_update_connector_phys(struct xe_device *xe,
				     struct intel_atomic_state *state,
				     ...
				     struct intel_pmdemand_state *pmdemand_state)
{
	...
	intel_pmdemand_update_phys_mask(xe, encoder, pmdemand_state,
					set_bit);
}

static void
intel_pmdemand_update_active_non_tc_phys(struct xe_device *xe,
					 struct intel_atomic_state *state,
					 struct intel_pmdemand_state *pmdemand_state)
{
	...
	for_each_oldnew_connector_in_state(&state->base, connector,
		...
		intel_pmdemand_update_connector_phys(xe, state,
...
static bool
intel_pmdemand_encoder_has_tc_phy(struct xe_device *xe,
				  struct intel_encoder *encoder)
{
...
intel_pmdemand_connector_needs_update(struct intel_atomic_state *state)
{
	...
	for_each_oldnew_connector_in_state(&state->base, connector,
		...
		if (old_encoder == new_encoder ||
		    (intel_pmdemand_encoder_has_tc_phy(xe, old_encoder) &&
		     intel_pmdemand_encoder_has_tc_phy(xe, new_encoder)))
			continue;
...
int intel_pmdemand_atomic_check(struct intel_atomic_state *state)
{
	...
	intel_pmdemand_update_max_ddiclk(xe, state, new_pmdemand_state);

	intel_pmdemand_update_active_non_tc_phys(xe, state, new_pmdemand_state);
...
static bool intel_pmdemand_check_prev_transaction(struct xe_device *xe)
{
	return !(intel_de_wait_for_clear(xe,
					 XELPDP_INITIATE_PMDEMAND_REQUEST(1),
					 XELPDP_PMDEMAND_REQ_ENABLE, 10) ||
		 intel_de_wait_for_clear(xe,
					 GEN12_DCPR_STATUS_1,
					 XELPDP_PMDEMAND_INFLIGHT_STATUS, 10));
}

void
intel_pmdemand_init_pmdemand_params(struct xe_device *xe,
				    struct intel_pmdemand_state *pmdemand_state)
{
	u32 reg1, reg2;
	...
	mutex_lock(&xe->display.pmdemand.lock);
	if (drm_WARN_ON(&xe->drm,
			!intel_pmdemand_check_prev_transaction(xe))) {
		memset(&pmdemand_state->params, 0,
		       sizeof(pmdemand_state->params));
		goto unlock;
	}

	reg1 = intel_de_read(xe, XELPDP_INITIATE_PMDEMAND_REQUEST(0));

	reg2 = intel_de_read(xe, XELPDP_INITIATE_PMDEMAND_REQUEST(1));
...
unlock:
	mutex_unlock(&xe->display.pmdemand.lock);
}

static bool intel_pmdemand_req_complete(struct xe_device *xe)
{
	return !(intel_de_read(xe, XELPDP_INITIATE_PMDEMAND_REQUEST(1)) &
		 XELPDP_PMDEMAND_REQ_ENABLE);
}

static void intel_pmdemand_wait(struct xe_device *xe)
{
	if (!wait_event_timeout(xe->display.pmdemand.waitqueue,
				intel_pmdemand_req_complete(xe),
...
void intel_pmdemand_program_dbuf(struct xe_device *xe,
				 u8 dbuf_slices)
{
	...
	intel_de_rmw(xe, XELPDP_INITIATE_PMDEMAND_REQUEST(0),
		     XELPDP_PMDEMAND_DBUFS_MASK,
		     REG_FIELD_PREP(XELPDP_PMDEMAND_DBUFS_MASK, dbufs));
	intel_de_rmw(xe, XELPDP_INITIATE_PMDEMAND_REQUEST(1), 0,
		     XELPDP_PMDEMAND_REQ_ENABLE);

	intel_pmdemand_wait(xe);
...
static void
intel_pmdemand_program_params(struct xe_device *xe,
			      ...
			      bool serialized)
{
	...
	if (reg1 != mod_reg1) {
		intel_de_write(xe, XELPDP_INITIATE_PMDEMAND_REQUEST(0),
			       mod_reg1);
		changed = true;
	}

	if (reg2 != mod_reg2) {
		intel_de_write(xe, XELPDP_INITIATE_PMDEMAND_REQUEST(1),
			       mod_reg2);
		changed = true;
	}
...
void intel_pmdemand_pre_plane_update(struct intel_atomic_state *state)
{
	...
	intel_pmdemand_program_params(xe, new_pmdemand_state,
				      old_pmdemand_state,
...
void intel_pmdemand_post_plane_update(struct intel_atomic_state *state)
{
	...
	intel_pmdemand_program_params(xe, new_pmdemand_state, NULL,
				      intel_atomic_global_state_is_serialized(state));
}
