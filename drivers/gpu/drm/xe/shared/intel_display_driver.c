...
void intel_display_driver_init_hw(struct xe_device *xe)
{
	...
	cdclk_state = to_intel_cdclk_state(xe->display.cdclk.obj.state);

	intel_update_cdclk(xe);
	intel_cdclk_dump_config(xe, &xe->display.cdclk.hw, "Current CDCLK");
	cdclk_state->logical = cdclk_state->actual = xe->display.cdclk.hw;

	intel_display_wa_apply(xe);
}
...
static void intel_mode_config_init(struct xe_device *xe)
{
	struct drm_mode_config *mode_config = &xe->drm.mode_config;

	drm_mode_config_init(&xe->drm);
	INIT_LIST_HEAD(&xe->display.global.obj_list);
	...
	mode_config->async_page_flip = HAS_ASYNC_FLIPS(xe);
	...
	if (IS_I845G(xe) || IS_I865G(xe)) {
		mode_config->cursor_width = IS_I845G(xe) ? 64 : 512;
		mode_config->cursor_height = 1023;
	} else if (IS_I830(xe) || IS_I85X(xe) ||
		   IS_I915G(xe) || IS_I915GM(xe)) {
...
static void intel_mode_config_cleanup(struct xe_device *xe)
{
	intel_atomic_global_obj_cleanup(xe);
	drm_mode_config_cleanup(&xe->drm);
}

static void intel_plane_possible_crtcs_init(struct xe_device *xe)
{
	struct intel_plane *plane;

	for_each_intel_plane(&xe->drm, plane) {
		struct intel_crtc *crtc = intel_crtc_for_pipe(xe,
							      plane->pipe);
...
void intel_display_driver_early_probe(struct xe_device *xe)
{
	...
	intel_dkl_phy_init(xe);
	intel_color_init_hooks(xe);
	intel_init_cdclk_hooks(xe);
	intel_audio_hooks_init(xe);
	intel_dpll_init_clock_hook(xe);
	intel_init_display_hooks(xe);
	intel_fdi_init_hook(xe);
	intel_dmc_wl_init(&xe->display);
}
...
int intel_display_driver_probe_noirq(struct xe_device *xe)
{
	...
	if (i915_inject_probe_failure(xe))
		return -ENODEV;

	if (HAS_DISPLAY(xe)) {
		ret = drm_vblank_init(&xe->drm,
				      INTEL_NUM_PIPES(xe));
		if (ret)
			return ret;
	}
	...
	ret = intel_vga_register(xe);
	if (ret)
		goto cleanup_bios;

	/* FIXME: completely on the wrong abstraction layer */
	ret = intel_power_domains_init(xe);
	if (ret < 0)
		goto cleanup_vga;

	intel_pmdemand_init_early(xe);

	intel_power_domains_init_hw(xe, false);
	...
	xe->display.wq.modeset = alloc_ordered_workqueue("i915_modeset", 0);
	xe->display.wq.flip = alloc_workqueue("i915_flip", WQ_HIGHPRI |
#ifdef __linux__
	...
	intel_mode_config_init(xe);

	ret = intel_cdclk_init(xe);
	if (ret)
		goto cleanup_vga_client_pw_domain_dmc;

	ret = intel_color_init(xe);
	if (ret)
		goto cleanup_vga_client_pw_domain_dmc;

	ret = intel_dbuf_init(xe);
	if (ret)
		goto cleanup_vga_client_pw_domain_dmc;

	ret = intel_bw_init(xe);
	if (ret)
		goto cleanup_vga_client_pw_domain_dmc;

	ret = intel_pmdemand_init(xe);
	if (ret)
		goto cleanup_vga_client_pw_domain_dmc;
...
static void set_display_access(struct xe_device *xe,
			       ...
			       struct task_struct *allowed_task)
{
	...
	intel_modeset_lock_ctx_retry(&ctx, NULL, 0, err) {
		err = drm_modeset_lock_all_ctx(&xe->drm, &ctx);
		if (err)
			continue;

		xe->display.access.any_task_allowed = any_task_allowed;
		xe->display.access.allowed_task = allowed_task;
	}
...
{
	set_display_access(xe, true, NULL);

	intel_hpd_enable_detection_work(xe);
}
...
{
	intel_hpd_disable_detection_work(xe);

	set_display_access(xe, false, current);
}
...
{
	set_display_access(xe, false, NULL);
}
...
bool intel_display_driver_check_access(struct xe_device *xe)
{
	...
	if (xe->display.access.any_task_allowed ||
	    xe->display.access.allowed_task == current)
		return true;
	...
	if (xe->display.access.allowed_task)
		snprintf(allowed_task, sizeof(allowed_task), "%s[%d]",
			 get_task_comm(comm, xe->display.access.allowed_task),
			 task_pid_vnr(xe->display.access.allowed_task));
...
int intel_display_driver_probe_nogem(struct xe_device *xe)
{
	...
	intel_panel_sanitize_ssc(xe);

	intel_pps_setup(display);

	intel_gmbus_setup(xe);

	drm_dbg_kms(&xe->drm, "%d display pipe%s available.\n",
		    INTEL_NUM_PIPES(xe),
		    INTEL_NUM_PIPES(xe) > 1 ? "s" : "");

	for_each_pipe(xe, pipe) {
		ret = intel_crtc_init(xe, pipe);
		if (ret)
			goto err_mode_config;
	}

	intel_plane_possible_crtcs_init(xe);
	intel_shared_dpll_init(xe);
	intel_fdi_pll_freq_update(xe);

	intel_update_czclk(xe);
	intel_display_driver_init_hw(xe);
	intel_dpll_update_ref_clks(xe);

	if (xe->display.cdclk.max_cdclk_freq == 0)
		intel_update_max_cdclk(xe);

	intel_hti_init(display);

	/* Just disable it once at startup */
	intel_vga_disable(xe);
	intel_setup_outputs(xe);
	...
	intel_modeset_setup_hw_state(xe, dev->mode_config.acquire_ctx);
	intel_acpi_assign_connector_fwnodes(display);
	drm_modeset_unlock_all(dev);

	intel_initial_plane_config(xe);
...
int intel_display_driver_probe(struct xe_device *xe)
{
	 ...
	 * during bootup.
	 */
	ret = intel_initial_commit(&xe->drm);
	if (ret)
		drm_dbg_kms(&xe->drm, "Initial modeset failed, %d\n", ret);

	intel_overlay_setup(xe);

	/* Only enable hotplug handling once the fbdev is fully set up. */
	intel_hpd_init(xe);

	skl_watermark_ipc_init(xe);
...
{
	struct intel_display *display = &xe->display;
	struct drm_printer p = drm_dbg_printer(&xe->drm, DRM_UT_KMS,
					       "i915 display info:");
	...
	intel_audio_register(xe);

	intel_display_debugfs_register(xe);
	 ...
	 * fbdev->async_cookie.
	 */
	drm_kms_helper_poll_init(&xe->drm);
	intel_hpd_poll_disable(xe);

	intel_fbdev_setup(xe);

	intel_display_device_info_print(DISPLAY_INFO(xe),
					DISPLAY_RUNTIME_INFO(xe), &p);
}
...
void intel_display_driver_remove(struct xe_device *xe)
{
	...
	flush_workqueue(xe->display.wq.flip);
	flush_workqueue(xe->display.wq.modeset);
...
void intel_display_driver_remove_noirq(struct xe_device *xe)
{
	...
	flush_workqueue(xe->unordered_wq);
	...
	intel_overlay_cleanup(xe);

	intel_gmbus_teardown(xe);

	destroy_workqueue(xe->display.wq.flip);
	destroy_workqueue(xe->display.wq.modeset);

	intel_fbc_cleanup(&xe->display);
}
...
void intel_display_driver_unregister(struct xe_device *xe)
{
	 ...
	 * the hotplug events.
	 */
	drm_kms_helper_poll_fini(&xe->drm);
...
int intel_display_driver_suspend(struct xe_device *xe)
{
	...
	state = drm_atomic_helper_suspend(&xe->drm);
	ret = PTR_ERR_OR_ZERO(state);
	if (ret)
		drm_err(&xe->drm, "Suspending crtc's failed with %i\n",
			ret);
	else
		xe->display.restore.modeset_state = state;
	return ret;
}

int
__intel_display_driver_resume(struct xe_device *xe,
			      struct drm_atomic_state *state,
			      struct drm_modeset_acquire_ctx *ctx)
{
	...
	intel_modeset_setup_hw_state(xe, ctx);
	intel_vga_redisable(xe);
	...
	drm_WARN_ON(&xe->drm, ret == -EDEADLK);
...
{
	struct drm_atomic_state *state = xe->display.restore.modeset_state;
	struct drm_modeset_acquire_ctx ctx;
	int ret;
	...
	xe->display.restore.modeset_state = NULL;
	if (state)
	...
	while (1) {
		ret = drm_modeset_lock_all_ctx(&xe->drm, &ctx);
		if (ret != -EDEADLK)
			break;
	...
	if (!ret)
		ret = __intel_display_driver_resume(xe, state, &ctx);
