					 ...
					 const struct drm_connector_state *conn_state)
{
	...
	if (connector->get_hw_state(connector)) {
		...
		I915_STATE_WARN(xe, !crtc_state,
				"connector enabled without attached crtc\n");

		if (!crtc_state)
			return;

		I915_STATE_WARN(xe, !crtc_state->hw.active,
		...
		I915_STATE_WARN(xe, conn_state->crtc != encoder->base.crtc,
				"attached encoder crtc differs from connector crtc\n");
	} else {
		I915_STATE_WARN(xe, crtc_state && crtc_state->hw.active,
				"attached crtc is active, but connector isn't\n");
		I915_STATE_WARN(xe, !crtc_state && conn_state->best_encoder,
		       ...
		       struct intel_crtc *crtc)
{
	...
	for_each_new_connector_in_state(&state->base, connector, new_conn_state, i) {
		...
		I915_STATE_WARN(to_xe(connector->dev), new_conn_state->best_encoder != encoder,
...
static void intel_pipe_config_sanity_check(const struct intel_crtc_state *crtc_state)
{
	...
	if (crtc_state->has_pch_encoder) {
		int fdi_dotclock = intel_dotclock_calculate(intel_fdi_link_freq(xe, crtc_state),
							    &crtc_state->fdi_m_n);
		 ...
		 */
		drm_WARN(&xe->drm, abs(fdi_dotclock - dotclock) > 1,
...
verify_encoder_state(struct intel_atomic_state *state)
{
	...
	for_each_intel_encoder(&xe->drm, encoder) {
		...
		drm_dbg_kms(&xe->drm, "[ENCODER:%d:%s]\n",
		...
		I915_STATE_WARN(xe, !!encoder->base.crtc != enabled,
		  ...
		  struct intel_crtc *crtc)
{
	...
	if (IS_I830(xe) && hw_crtc_state->hw.active)
	...
	I915_STATE_WARN(xe, crtc->active != sw_crtc_state->hw.active,
	...
	for_each_encoder_on_crtc(dev, &primary_crtc->base, encoder) {
		...
		active = encoder->get_hw_state(encoder, &pipe);
		I915_STATE_WARN(xe, active != sw_crtc_state->hw.active,
				"[ENCODER:%i] active %i with crtc active %i\n",
				encoder->base.base.id, active,
				sw_crtc_state->hw.active);

		I915_STATE_WARN(xe, active && primary_crtc->pipe != pipe,
	...
	if (!intel_pipe_config_compare(sw_crtc_state,
				       hw_crtc_state, false)) {
		I915_STATE_WARN(xe, 1, "pipe state doesn't match!\n");
