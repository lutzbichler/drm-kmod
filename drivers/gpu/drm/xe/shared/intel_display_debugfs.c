...
static inline struct xe_device *node_to_xe(struct drm_info_node *node)
{
	return to_xe(node->minor->dev);
}
...
static int i915_frontbuffer_tracking(struct seq_file *m, void *unused)
{
	...
	spin_lock(&xe->display.fb_tracking.lock);

	seq_printf(m, "FB tracking busy bits: 0x%08x\n",
		   xe->display.fb_tracking.busy_bits);

	seq_printf(m, "FB tracking flip bits: 0x%08x\n",
		   xe->display.fb_tracking.flip_bits);

	spin_unlock(&xe->display.fb_tracking.lock);
...
static int i915_sr_status(struct seq_file *m, void *unused)
{
	...
	if (DISPLAY_VER(xe) >= 9)
		...
		sr_enabled = intel_de_read(xe, WM1_LP_ILK) & WM_LP_ENABLE;
	else if (IS_I965GM(xe) || IS_G4X(xe) ||
		 IS_I945G(xe) || IS_I945GM(xe))
		sr_enabled = intel_de_read(xe, FW_BLC_SELF) & FW_BLC_SELF_EN;
	else if (IS_I915GM(xe))
		sr_enabled = intel_de_read(xe, INSTPM) & INSTPM_SELF_EN;
	else if (IS_PINEVIEW(xe))
		sr_enabled = intel_de_read(xe, DSPFW3(xe)) & PINEVIEW_SELF_REFRESH_EN;
	else if (IS_VALLEYVIEW(xe) || IS_CHERRYVIEW(xe))
		sr_enabled = intel_de_read(xe, FW_BLC_SELF_VLV) & FW_CSPWRDWNEN;
...
static int i915_gem_framebuffer_info(struct seq_file *m, void *data)
{
	...
	fbdev_fb = intel_fbdev_framebuffer(xe->display.fbdev.fbdev);
	if (fbdev_fb) {
	...
	mutex_lock(&xe->drm.mode_config.fb_lock);
	drm_for_each_fb(drm_fb, &xe->drm) {
	...
	mutex_unlock(&xe->drm.mode_config.fb_lock);
...
static int i915_power_domain_info(struct seq_file *m, void *unused)
{
	...
	intel_display_power_debug(xe, m);
...
static void intel_crtc_info(struct seq_file *m, struct intel_crtc *crtc)
{
	...
	for_each_intel_encoder_mask(&xe->drm, encoder,
...
static int i915_shared_dplls_info(struct seq_file *m, void *unused)
{
	...
	drm_printf(&p, "PLL refclks: non-SSC: %d kHz, SSC: %d kHz\n",
		   xe->display.dpll.ref_clks.nssc,
		   xe->display.dpll.ref_clks.ssc);
	...
	for_each_shared_dpll(xe, pll, i) {
		...
		intel_dpll_dump_hw_state(xe, &p, &pll->state.hw_state);
	}
	drm_modeset_unlock_all(&xe->drm);
...
static bool
intel_lpsp_power_well_enabled(struct xe_device *xe,
			      enum i915_power_well_id power_well_id)
{
	...
	wakeref = intel_runtime_pm_get(&xe->runtime_pm);
	is_enabled = intel_display_power_well_is_enabled(xe,
							 power_well_id);
	intel_runtime_pm_put(&xe->runtime_pm, wakeref);
...
static int i915_lpsp_status(struct seq_file *m, void *unused)
{
	...
	if (DISPLAY_VER(xe) >= 13 || IS_DISPLAY_VER(xe, 9, 10)) {
		lpsp_enabled = !intel_lpsp_power_well_enabled(xe, SKL_DISP_PW_2);
	} else if (IS_DISPLAY_VER(xe, 11, 12)) {
		lpsp_enabled = !intel_lpsp_power_well_enabled(xe, ICL_DISP_PW_3);
	} else if (IS_HASWELL(xe) || IS_BROADWELL(xe)) {
		lpsp_enabled = !intel_lpsp_power_well_enabled(xe, HSW_DISP_PW_GLOBAL);
	} else {
...
{
	struct xe_device *xe = filp->private_data;
	struct intel_crtc *crtc;
	int ret;
	bool reset;
	...
	intel_fbc_reset_underrun(&xe->display);
...
void intel_display_debugfs_register(struct xe_device *xe)
{
	...
	intel_cdclk_debugfs_register(xe);
	intel_dmc_debugfs_register(xe);
	intel_fbc_debugfs_register(display);
	intel_hpd_debugfs_register(xe);
	intel_opregion_debugfs_register(display);
	intel_psr_debugfs_register(display);
	intel_wm_debugfs_register(xe);
...
static int i915_hdcp_sink_capability_show(struct seq_file *m, void *data)
{
	...
	ret = drm_modeset_lock_single_interruptible(&xe->drm.mode_config.connection_mutex);
	if (ret)
		return ret;
...
static int i915_lpsp_capability_show(struct seq_file *m, void *data)
{
	...
	else if (IS_DISPLAY_VER(xe, 9, 10))
					  ...
					  size_t len, loff_t *offp)
{
	...
	drm_dbg(&xe->drm, "Got %s for DSC Enable\n",
...
out:	drm_modeset_unlock(&xe->drm.mode_config.connection_mutex);
					     ...
					     size_t len, loff_t *offp)
{
	...
	drm_dbg(&xe->drm, "Got %s for DSC Fractional BPP Enable\n",
