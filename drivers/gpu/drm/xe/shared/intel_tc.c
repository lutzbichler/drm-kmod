...
static struct xe_device *tc_to_xe(struct intel_tc_port *tc)
{
	return to_xe(tc->dig_port->base.base.dev);
}
...
bool intel_tc_cold_requires_aux_pw(struct intel_digital_port *dig_port)
{
	...
	return tc_phy_cold_off_domain(tc) ==
	       intel_display_power_legacy_aux_domain(xe, dig_port->aux_ch);
}

static intel_wakeref_t
__tc_cold_block(struct intel_tc_port *tc, enum intel_display_power_domain *domain)
{
	struct xe_device *xe = tc_to_xe(tc);

	*domain = tc_phy_cold_off_domain(tc);

	return intel_display_power_get(xe, *domain);
}
		  ...
		  intel_wakeref_t wakeref)
{
	...
	intel_display_power_put(xe, domain, wakeref);
}
...
#if IS_ENABLED(CONFIG_DRM_I915_DEBUG_RUNTIME_PM)
	drm_WARN_ON(&tc_to_xe(tc)->drm, tc->lock_power_domain != domain);
#endif
...
assert_display_core_power_enabled(struct intel_tc_port *tc)
{
	...
	drm_WARN_ON(&xe->drm,
		    !intel_display_power_is_enabled(xe, POWER_DOMAIN_DISPLAY_CORE));
}
...
assert_tc_cold_blocked(struct intel_tc_port *tc)
{
	...
	enabled = intel_display_power_is_enabled(xe,
...
assert_tc_port_power_enabled(struct intel_tc_port *tc)
{
	...
	drm_WARN_ON(&xe->drm,
		    !intel_display_power_is_enabled(xe, tc_port_power_domain(tc)));
}
...
static u32 intel_tc_port_get_lane_mask(struct intel_digital_port *dig_port)
{
	...
	lane_mask = intel_de_read(xe, PORT_TX_DFLEXDPSP(tc->phy_fia));

	drm_WARN_ON(&xe->drm, lane_mask == 0xffffffff);
...
u32 intel_tc_port_get_pin_assignment_mask(struct intel_digital_port *dig_port)
{
	...
	pin_mask = intel_de_read(xe, PORT_TX_DFLEXPA1(tc->phy_fia));

	drm_WARN_ON(&xe->drm, pin_mask == 0xffffffff);
	assert_tc_cold_blocked(tc);
...
static int lnl_tc_port_get_max_lane_count(struct intel_digital_port *dig_port)
{
	...
	with_intel_display_power(xe, POWER_DOMAIN_DISPLAY_CORE, wakeref)
		val = intel_de_read(xe, TCSS_DDI_STATUS(tc_port));
				      ...
				      int required_lanes)
{
	...
	val = intel_de_read(xe, PORT_TX_DFLEXDPMLE1(tc->phy_fia));
	...
	intel_de_write(xe, PORT_TX_DFLEXDPMLE1(tc->phy_fia), val);
}
				      ...
				      u32 live_status_mask)
{
	...
	drm_WARN_ON(&xe->drm, tc->mode != TC_PORT_DISCONNECTED);
...
static u32 icl_tc_phy_hpd_live_status(struct intel_tc_port *tc)
{
	struct xe_device *xe = tc_to_xe(tc);
	struct intel_digital_port *dig_port = tc->dig_port;
	u32 isr_bit = xe->display.hotplug.pch_hpd[dig_port->base.hpd_pin];
	intel_wakeref_t wakeref;
	u32 fia_isr;
	u32 pch_isr;
	u32 mask = 0;

	with_intel_display_power(xe, tc_phy_cold_off_domain(tc), wakeref) {
		fia_isr = intel_de_read(xe, PORT_TX_DFLEXDPSP(tc->phy_fia));
		pch_isr = intel_de_read(xe, SDEISR);
	}
...
static bool icl_tc_phy_is_ready(struct intel_tc_port *tc)
{
	...
	val = intel_de_read(xe, PORT_TX_DFLEXDPPMS(tc->phy_fia));
	if (val == 0xffffffff) {
		drm_dbg_kms(&xe->drm,
				      ...
				      bool take)
{
	...
	val = intel_de_read(xe, PORT_TX_DFLEXDPCSSS(tc->phy_fia));
	if (val == 0xffffffff) {
		drm_dbg_kms(&xe->drm,
	...
	intel_de_write(xe, PORT_TX_DFLEXDPCSSS(tc->phy_fia), val);
						...
						int required_lanes)
{
	...
	if (tc->mode == TC_PORT_LEGACY) {
		drm_WARN_ON(&xe->drm, max_lanes != 4);
		return true;
	}

	drm_WARN_ON(&xe->drm, tc->mode != TC_PORT_DP_ALT);
	...
	if (!(tc_phy_hpd_live_status(tc) & BIT(TC_PORT_DP_ALT))) {
		drm_dbg_kms(&xe->drm, "Port %s: PHY sudden disconnect\n",
			    tc->port_name);
		return false;
	}
			       ...
			       int required_lanes)
{
	...
	if ((!tc_phy_is_ready(tc) ||
	    ...
	    !drm_WARN_ON(&xe->drm, tc->mode == TC_PORT_LEGACY)) {
		drm_dbg_kms(&xe->drm, "Port %s: can't take PHY ownership (ready %s)\n",
			    tc->port_name,
...
static void tgl_tc_phy_init(struct intel_tc_port *tc)
{
	...
	with_intel_display_power(xe, tc_phy_cold_off_domain(tc), wakeref)
		val = intel_de_read(xe, PORT_TX_DFLEXDPSP(FIA1));

	drm_WARN_ON(&xe->drm, val == 0xffffffff);
...
static u32 adlp_tc_phy_hpd_live_status(struct intel_tc_port *tc)
{
	...
	u32 cpu_isr_bits = xe->display.hotplug.hpd[hpd_pin];
	u32 pch_isr_bit = xe->display.hotplug.pch_hpd[hpd_pin];
	intel_wakeref_t wakeref;
	u32 cpu_isr;
	u32 pch_isr;
	u32 mask = 0;
	...
	with_intel_display_power(xe, POWER_DOMAIN_DISPLAY_CORE, wakeref) {
		cpu_isr = intel_de_read(xe, GEN11_DE_HPD_ISR);
		pch_isr = intel_de_read(xe, SDEISR);
	}
				       ...
				       bool take)
{
	...
	intel_de_rmw(xe, DDI_BUF_CTL(port), DDI_BUF_CTL_TC_PHY_OWNERSHIP,
...
static void adlp_tc_phy_get_hw_state(struct intel_tc_port *tc)
{
	...
	port_wakeref = intel_display_power_get(xe, port_power_domain);

	tc->mode = tc_phy_get_current_mode(tc);
	if (tc->mode != TC_PORT_DISCONNECTED)
		tc->lock_wakeref = tc_cold_block(tc);

	intel_display_power_put(xe, port_power_domain, port_wakeref);
}
...
static bool adlp_tc_phy_connect(struct intel_tc_port *tc, int required_lanes)
{
	...
	if (!adlp_tc_phy_take_ownership(tc, true) &&
	    !drm_WARN_ON(&xe->drm, tc->mode == TC_PORT_LEGACY)) {
		drm_dbg_kms(&xe->drm, "Port %s: can't take PHY ownership\n",
			    tc->port_name);
	...
	if (!tc_phy_is_ready(tc) &&
	    !drm_WARN_ON(&xe->drm, tc->mode == TC_PORT_LEGACY)) {
		drm_dbg_kms(&xe->drm, "Port %s: PHY not ready\n",
			    tc->port_name);
		goto out_release_phy;
	}
...
static u32 xelpdp_tc_phy_hpd_live_status(struct intel_tc_port *tc)
{
	...
	struct intel_digital_port *dig_port = tc->dig_port;
	enum hpd_pin hpd_pin = dig_port->base.hpd_pin;
	u32 pica_isr_bits = xe->display.hotplug.hpd[hpd_pin];
	u32 pch_isr_bit = xe->display.hotplug.pch_hpd[hpd_pin];
	intel_wakeref_t wakeref;
	u32 pica_isr;
	u32 pch_isr;
	...
	with_intel_display_power(xe, POWER_DOMAIN_DISPLAY_CORE, wakeref) {
		pica_isr = intel_de_read(xe, PICAINTERRUPT_ISR);
		pch_isr = intel_de_read(xe, SDEISR);
	}
...
xelpdp_tc_phy_tcss_power_is_enabled(struct intel_tc_port *tc)
{
	struct xe_device *xe = tc_to_xe(tc);
	enum port port = tc->dig_port->base.port;
	i915_reg_t reg = XELPDP_PORT_BUF_CTL1(xe, port);

	assert_tc_cold_blocked(tc);

	return intel_de_read(xe, reg) & XELPDP_TCSS_POWER_STATE;
}
...
out_disable:
	if (drm_WARN_ON(&xe->drm, tc->mode == TC_PORT_LEGACY))
		return false;
...
static bool xelpdp_tc_phy_is_owned(struct intel_tc_port *tc)
{
	...
	return intel_de_read(xe, reg) & XELPDP_TC_PHY_OWNERSHIP;
}
...
static u32 tc_phy_hpd_live_status(struct intel_tc_port *tc)
{
	...
	drm_WARN_ON_ONCE(&xe->drm, hweight32(mask) > 1);
				      ...
				      bool phy_is_ready, bool phy_is_owned)
{
	...
	drm_WARN_ON(&xe->drm, phy_is_owned && !phy_is_ready);
...
static bool tc_phy_wait_for_ready(struct intel_tc_port *tc)
{
	...
	if (wait_for(tc_phy_is_ready(tc), 500)) {
		drm_err(&xe->drm, "Port %s: timeout waiting for PHY ready\n",
			tc->port_name);
...
tc_phy_get_current_mode(struct intel_tc_port *tc)
{
	...
	if (!tc_phy_is_ready_and_owned(tc, phy_is_ready, phy_is_owned)) {
		...
		drm_WARN_ON(&xe->drm, live_mode == TC_PORT_TBT_ALT);
				     ...
				     int required_lanes, bool force_disconnect)
{
	...
	if (!intel_tc_cold_requires_aux_pw(dig_port)) {
		...
		aux_powered = intel_display_power_is_enabled(xe, aux_domain);
		drm_WARN_ON(&xe->drm, aux_powered);
	}
	...
	drm_dbg_kms(&xe->drm, "Port %s: TC port mode reset (%s -> %s)\n",
		    tc->port_name,
...
static bool tc_port_is_enabled(struct intel_tc_port *tc)
{
	...
	return intel_de_read(xe, DDI_BUF_CTL(dig_port->base.port)) &
	       DDI_BUF_CTL_ENABLE;
}
...
void intel_tc_port_init_mode(struct intel_digital_port *dig_port)
{
	...
	drm_WARN_ON(&xe->drm, tc->mode != TC_PORT_DISCONNECTED);
	drm_WARN_ON(&xe->drm, tc->lock_wakeref);
	drm_WARN_ON(&xe->drm, tc->link_refcount);
	...
	if (!tc_port_is_enabled(tc)) {
		...
		drm_WARN_ON(&xe->drm, !tc->legacy_port);
		drm_err(&xe->drm,
				 ...
				 const struct intel_crtc_state *crtc_state)
{
	...
	drm_WARN_ON(&xe->drm, tc->link_refcount != 1);
	...
	drm_dbg_kms(&xe->drm, "Port %s: sanitize mode (%s)\n",
		    tc->port_name,
		    tc_port_mode_name(tc->mode));
			     ...
			     struct drm_modeset_acquire_ctx *ctx)
{
	...
	ret = drm_modeset_lock(&xe->drm.mode_config.connection_mutex, ctx);
	if (ret)
		return ret;
...
static int reset_link(struct intel_tc_port *tc)
{
	...
	_state = drm_atomic_state_alloc(&xe->drm);
	if (!_state)
		return -ENOMEM;
				 ...
				 int required_lanes)
{
	...
	drm_WARN_ON(&xe->drm, tc->mode == TC_PORT_DISCONNECTED);
	drm_WARN_ON(&xe->drm, tc->mode != TC_PORT_TBT_ALT &&
				!tc_phy_is_owned(tc));
}
...
int intel_tc_port_init(struct intel_digital_port *dig_port, bool is_legacy)
{
	...
	if (drm_WARN_ON(&xe->drm, tc_port == TC_PORT_NONE))
		return -EINVAL;
