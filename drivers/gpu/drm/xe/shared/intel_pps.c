...
intel_wakeref_t intel_pps_lock(struct intel_dp *intel_dp)
{
	 ...
	 */
	wakeref = intel_display_power_get(xe, POWER_DOMAIN_DISPLAY_CORE);
...
vlv_power_sequencer_kick(struct intel_dp *intel_dp)
{
	 ...
	 */
	if (!pll_enabled) {
		release_cl_override = IS_CHERRYVIEW(xe) &&
			!chv_phy_powergate_ch(xe, phy, ch, true);

		if (vlv_force_pll_on(xe, pipe, vlv_get_dpll(xe))) {
			drm_err(display->drm,
	...
	if (!pll_enabled) {
		vlv_force_pll_off(xe, pipe);

		if (release_cl_override)
			chv_phy_powergate_ch(xe, phy, ch, false);
	}
}
...
void intel_pps_reset_all(struct intel_display *display)
{
	...
	if (drm_WARN_ON(display->drm, !IS_LP(xe)))
		return;
...
bool intel_pps_vdd_on_unlocked(struct intel_dp *intel_dp)
{
	...
	intel_dp->pps.vdd_wakeref = intel_display_power_get(xe,
...
void intel_pps_vdd_on(struct intel_dp *intel_dp)
{
	...
	I915_STATE_WARN(xe, !vdd, "[ENCODER:%d:%s] %s VDD already requested on\n",
...
void intel_pps_vdd_off_unlocked(struct intel_dp *intel_dp, bool sync)
{
	...
	I915_STATE_WARN(xe, !intel_dp->pps.want_panel_vdd,
...
void intel_pps_setup(struct intel_display *display)
{
	...
	if (HAS_PCH_SPLIT(xe) || IS_GEMINILAKE(xe) || IS_BROXTON(xe))
...
void assert_pps_unlocked(struct intel_display *display, enum pipe pipe)
{
	...
	if (HAS_PCH_SPLIT(xe)) {
		...
		case PANEL_PORT_SELECT_LVDS:
			intel_lvds_port_enabled(xe, PCH_LVDS, &panel_pipe);
			break;
		case PANEL_PORT_SELECT_DPA:
			g4x_dp_port_enabled(xe, DP_A, PORT_A, &panel_pipe);
			break;
		case PANEL_PORT_SELECT_DPC:
			g4x_dp_port_enabled(xe, PCH_DP_C, PORT_C, &panel_pipe);
			break;
		case PANEL_PORT_SELECT_DPD:
			g4x_dp_port_enabled(xe, PCH_DP_D, PORT_D, &panel_pipe);
			break;
		default:
		...
		intel_lvds_port_enabled(xe, LVDS, &panel_pipe);
	}
	...
	I915_STATE_WARN(xe, panel_pipe == pipe && locked,
