...
static u32 intel_cursor_base(const struct intel_plane_state *plane_state)
{
	...
	if (DISPLAY_INFO(xe)->cursor_needs_physical)
...
static int intel_cursor_check_surface(struct intel_plane_state *plane_state)
{
	...
	if (HAS_GMCH(xe) && rotation & DRM_MODE_ROTATE_180) {
...
{
	const struct drm_framebuffer *fb = plane_state->hw.fb;
	struct xe_device *xe = to_xe(plane_state->uapi.plane->dev);
	const struct drm_rect src = plane_state->uapi.src;
	const struct drm_rect dst = plane_state->uapi.dst;
	int ret;

	if (fb && fb->modifier != DRM_FORMAT_MOD_LINEAR) {
		drm_dbg_kms(&xe->drm, "cursor cannot be tiled\n");
		return -EINVAL;
	}
			     ...
			     struct intel_plane_state *plane_state)
{
	...
	drm_WARN_ON(&xe->drm, plane_state->uapi.visible &&
	...
	case 2048:
		break;
	default:
		 drm_dbg_kms(&xe->drm, "Invalid cursor stride (%u)\n",
			     fb->pitches[0]);
		return -EINVAL;
	}
				   ...
				   const struct intel_plane_state *plane_state)
{
	...
	if (plane->cursor.base != base ||
		...
		intel_de_write_fw(xe, CURCNTR(xe, PIPE_A), 0);
		intel_de_write_fw(xe, CURBASE(xe, PIPE_A), base);
		intel_de_write_fw(xe, CURSIZE(xe, PIPE_A), size);
		intel_de_write_fw(xe, CURPOS(xe, PIPE_A), pos);
		intel_de_write_fw(xe, CURCNTR(xe, PIPE_A), cntl);
				     ...
				     enum pipe *pipe)
{
	...
	ret = intel_de_read(xe, CURCNTR(xe, PIPE_A)) & CURSOR_ENABLE;
			   ...
			   const struct intel_plane_state *plane_state)
{
	...
	if (IS_SANDYBRIDGE(xe) || IS_IVYBRIDGE(xe))
			     ...
			     struct intel_plane_state *plane_state)
{
	 ...
	 */
	if (IS_CHERRYVIEW(xe) && pipe == PIPE_C &&
					      ...
					      const struct intel_crtc_state *crtc_state)
{
	...
	intel_de_write_fw(xe, SEL_FETCH_CUR_CTL(pipe), 0);
}
			   ...
			   const struct intel_plane_state *plane_state)
{
	...
	intel_de_write_fw(xe, SEL_FETCH_CUR_CTL(pipe), ctl);

	intel_de_write(xe, CURPOS_ERLY_TPT(xe, pipe),
	...
	if (drm_rect_height(&plane_state->psr2_sel_fetch_area) > 0) {
		if (crtc_state->enable_psr2_su_region_et) {
			u32 val = intel_cursor_position(crtc_state, plane_state,
				true);
			intel_de_write_fw(xe,
					  CURPOS_ERLY_TPT(xe, pipe),
					  val);
		}

		intel_de_write_fw(xe, SEL_FETCH_CUR_CTL(pipe),
				  plane_state->ctl);
	} else {
		/* Wa_16021440873 */
		if (crtc_state->enable_psr2_su_region_et)
				...
				const struct intel_crtc_state *crtc_state)
{
	...
	for (level = 0; level < xe->display.wm.num_levels; level++)
		intel_de_write_fw(xe, CUR_WM(pipe, level),
				  skl_cursor_wm_reg_val(skl_plane_wm_level(pipe_wm, plane_id, level)));

	intel_de_write_fw(xe, CUR_WM_TRANS(pipe),
			  skl_cursor_wm_reg_val(skl_plane_trans_wm(pipe_wm, plane_id)));

	if (HAS_HW_SAGV_WM(xe)) {
		const struct skl_plane_wm *wm = &pipe_wm->planes[plane_id];

		intel_de_write_fw(xe, CUR_WM_SAGV(pipe),
				  skl_cursor_wm_reg_val(&wm->sagv.wm0));
		intel_de_write_fw(xe, CUR_WM_SAGV_TRANS(pipe),
				  skl_cursor_wm_reg_val(&wm->sagv.trans_wm));
	}

	intel_de_write_fw(xe, CUR_BUF_CFG(pipe),
	...
	if (plane->cursor.base != base ||
		...
		if (HAS_CUR_FBC(xe))
			intel_de_write_fw(xe,
					  CUR_FBC_CTL(xe, pipe),
					  fbc_ctl);
		intel_de_write_fw(xe, CURCNTR(xe, pipe), cntl);
		intel_de_write_fw(xe, CURPOS(xe, pipe), pos);
		intel_de_write_fw(xe, CURBASE(xe, pipe), base);
				     ...
				     enum pipe *pipe)
{
	...
	val = intel_de_read(xe, CURCNTR(xe, plane->pipe));
			   ...
			   struct drm_modeset_acquire_ctx *ctx)
{
	...
	if (!drm_WARN_ON(&xe->drm, drm_crtc_vblank_get(&crtc->base))) {
		/*
...
{
	struct xe_device *xe = to_xe(plane->base.dev);
	const struct drm_mode_config *config = &xe->drm.mode_config;
	...
	for (size = 64; size <= max_size; size *= 2) {
		if (drm_WARN_ON(&xe->drm, num_hints >= ARRAY_SIZE(hints)))
			break;
			  ...
			  enum pipe pipe)
{
	...
	if (IS_I845G(xe) || IS_I865G(xe) || HAS_CUR_FBC(xe))
		cursor->cursor.size = ~0;

	modifiers = intel_fb_plane_get_modifiers(xe, INTEL_PLANE_CAP_NONE);

	ret = drm_universal_plane_init(&xe->drm, &cursor->base,
	...
	zpos = DISPLAY_RUNTIME_INFO(xe)->num_sprites[pipe] + 1;
