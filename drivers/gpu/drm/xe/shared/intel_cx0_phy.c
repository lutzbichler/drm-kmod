...
bool intel_encoder_is_c10phy(struct intel_encoder *encoder)
{
	...
	if ((IS_LUNARLAKE(xe) || IS_METEORLAKE(xe)) && phy < PHY_C)
		return true;
...
static void
assert_dc_off(struct xe_device *xe)
{
	bool enabled;

	enabled = intel_display_power_is_enabled(xe, POWER_DOMAIN_DC_OFF);
	drm_WARN_ON(&xe->drm, !enabled);
}
...
static void intel_cx0_program_msgbus_timer(struct intel_encoder *encoder)
{
	...
	for_each_cx0_lane_in_mask(INTEL_CX0_BOTH_LANES, lane)
		intel_de_rmw(xe,
			     XELPDP_PORT_MSGBUS_TIMER(xe, encoder->port, lane),
...
static intel_wakeref_t intel_cx0_phy_transaction_begin(struct intel_encoder *encoder)
{
	...
	wakeref = intel_display_power_get(xe, POWER_DOMAIN_DC_OFF);
...
static void intel_cx0_phy_transaction_end(struct intel_encoder *encoder, intel_wakeref_t wakeref)
{
	...
	intel_display_power_put(xe, POWER_DOMAIN_DC_OFF, wakeref);
}
					    ...
					    int lane)
{
	...
	intel_de_rmw(xe, XELPDP_PORT_P2M_MSGBUS_STATUS(xe, encoder->port, lane),
...
static void intel_cx0_bus_reset(struct intel_encoder *encoder, int lane)
{
	...
	intel_de_write(xe, XELPDP_PORT_M2P_MSGBUS_CTL(xe, port, lane),
		       XELPDP_PORT_M2P_TRANSACTION_RESET);

	if (intel_de_wait_for_clear(xe, XELPDP_PORT_M2P_MSGBUS_CTL(xe, port, lane),
				    XELPDP_PORT_M2P_TRANSACTION_RESET,
				    XELPDP_MSGBUS_TIMEOUT_SLOW)) {
		drm_err_once(&xe->drm, "Failed to bring PHY %c to idle.\n", phy_name(phy));
		return;
	}
				  ...
				  int command, int lane, u32 *val)
{
	...
	if (intel_de_wait_custom(xe,
				 XELPDP_PORT_P2M_MSGBUS_STATUS(xe, port, lane),
				 XELPDP_PORT_P2M_RESPONSE_READY,
				 XELPDP_PORT_P2M_RESPONSE_READY,
				 XELPDP_MSGBUS_TIMEOUT_FAST_US,
				 XELPDP_MSGBUS_TIMEOUT_SLOW, val)) {
		drm_dbg_kms(&xe->drm, "PHY %c Timeout waiting for message ACK. Status: 0x%x\n",
			    phy_name(phy), *val);

		if (!(intel_de_read(xe, XELPDP_PORT_MSGBUS_TIMER(xe, port, lane)) &
	...
	if (*val & XELPDP_PORT_P2M_ERROR_SET) {
		drm_dbg_kms(&xe->drm, "PHY %c Error occurred during %s command. Status: 0x%x\n", phy_name(phy),
	...
	if (REG_FIELD_GET(XELPDP_PORT_P2M_COMMAND_TYPE_MASK, *val) != command) {
		drm_dbg_kms(&xe->drm, "PHY %c Not a %s response. MSGBUS Status: 0x%x.\n", phy_name(phy),
			    command == XELPDP_PORT_P2M_COMMAND_READ_ACK ? "read" : "write", *val);
		intel_cx0_bus_reset(encoder, lane);
		return -EINVAL;
	}
			   ...
			   int lane, u16 addr)
{
	...
	drm_err_once(&xe->drm, "PHY %c Read %04x failed after %d retries.\n",
		     phy_name(phy), addr, i);
				  ...
				  int lane, u16 addr, u8 data, bool committed)
{
	...
	if (committed) {
		...
		if (ack < 0)
			return ack;
	} else if ((intel_de_read(xe, XELPDP_PORT_P2M_MSGBUS_STATUS(xe, port, lane)) &
				     ...
				     const struct intel_crtc_state *crtc_state)
{
	...
	if (drm_WARN_ON_ONCE(&xe->drm, !trans)) {
				    ...
				    struct intel_encoder *encoder)
{
	...
	drm_WARN_ON(&xe->drm, ARRAY_SIZE(pll_state->c10.pll) < 9);
	for (i = 4; i < 9; i++)
...
static void intel_c10_pll_program(struct xe_device *xe,
				  const struct intel_crtc_state *crtc_state,
				  struct intel_encoder *encoder)
{
...
static void intel_c10pll_dump_hw_state(struct xe_device *xe,
				       const struct intel_c10pll_state *hw_state)
{
	...
	drm_dbg_kms(&xe->drm, "c10pll_hw_state: fracen: %s, ",
		    str_yes_no(fracen));
	...
	if (fracen) {
		...
		drm_dbg_kms(&xe->drm, "quot: %u, rem: %u, den: %u,\n",
	...
	drm_dbg_kms(&xe->drm, "c10pll_rawhw_state:");
	drm_dbg_kms(&xe->drm, "tx: 0x%x, cmn: 0x%x\n", hw_state->tx, hw_state->cmn);

	BUILD_BUG_ON(ARRAY_SIZE(hw_state->pll) % 4);
	for (i = 0; i < ARRAY_SIZE(hw_state->pll); i = i + 4)
		drm_dbg_kms(&xe->drm, "pll[%d] = 0x%x, pll[%d] = 0x%x, pll[%d] = 0x%x, pll[%d] = 0x%x\n",
	...
	for (i = 0; i < ARRAY_SIZE(pll_state->tx); i++) {
		if (cntx)
			pll_state->tx[i] = intel_c20_sram_read(encoder,
							       INTEL_CX0_LANE0,
							       PHY_C20_B_TX_CNTX_CFG(xe, i));
		else
			pll_state->tx[i] = intel_c20_sram_read(encoder,
							       INTEL_CX0_LANE0,
							       PHY_C20_A_TX_CNTX_CFG(xe, i));
	}
	...
	for (i = 0; i < ARRAY_SIZE(pll_state->cmn); i++) {
		if (cntx)
			pll_state->cmn[i] = intel_c20_sram_read(encoder,
								INTEL_CX0_LANE0,
								PHY_C20_B_CMN_CNTX_CFG(xe, i));
		else
			pll_state->cmn[i] = intel_c20_sram_read(encoder,
								INTEL_CX0_LANE0,
								PHY_C20_A_CMN_CNTX_CFG(xe, i));
	}
		...
		for (i = 0; i < ARRAY_SIZE(pll_state->mpllb); i++) {
			if (cntx)
				pll_state->mpllb[i] = intel_c20_sram_read(encoder,
									  INTEL_CX0_LANE0,
									  PHY_C20_B_MPLLB_CNTX_CFG(xe, i));
			else
				pll_state->mpllb[i] = intel_c20_sram_read(encoder,
									  INTEL_CX0_LANE0,
									  PHY_C20_A_MPLLB_CNTX_CFG(xe, i));
		}
	} else {
		/* MPLLA configuration */
		for (i = 0; i < ARRAY_SIZE(pll_state->mplla); i++) {
			if (cntx)
				pll_state->mplla[i] = intel_c20_sram_read(encoder,
									  INTEL_CX0_LANE0,
									  PHY_C20_B_MPLLA_CNTX_CFG(xe, i));
			else
				pll_state->mplla[i] = intel_c20_sram_read(encoder,
									  INTEL_CX0_LANE0,
									  PHY_C20_A_MPLLA_CNTX_CFG(xe, i));
		}
	}
...
static void intel_c20pll_dump_hw_state(struct xe_device *xe,
				       const struct intel_c20pll_state *hw_state)
{
	int i;

	drm_dbg_kms(&xe->drm, "c20pll_hw_state:\n");
	drm_dbg_kms(&xe->drm, "tx[0] = 0x%.4x, tx[1] = 0x%.4x, tx[2] = 0x%.4x\n",
		    hw_state->tx[0], hw_state->tx[1], hw_state->tx[2]);
	drm_dbg_kms(&xe->drm, "cmn[0] = 0x%.4x, cmn[1] = 0x%.4x, cmn[2] = 0x%.4x, cmn[3] = 0x%.4x\n",
		    hw_state->cmn[0], hw_state->cmn[1], hw_state->cmn[2], hw_state->cmn[3]);

	if (intel_c20phy_use_mpllb(hw_state)) {
		for (i = 0; i < ARRAY_SIZE(hw_state->mpllb); i++)
			drm_dbg_kms(&xe->drm, "mpllb[%d] = 0x%.4x\n", i, hw_state->mpllb[i]);
	} else {
		for (i = 0; i < ARRAY_SIZE(hw_state->mplla); i++)
			drm_dbg_kms(&xe->drm, "mplla[%d] = 0x%.4x\n", i, hw_state->mplla[i]);
	}
}

void intel_cx0pll_dump_hw_state(struct xe_device *xe,
				const struct intel_cx0pll_state *hw_state)
{
	if (hw_state->use_c10)
		intel_c10pll_dump_hw_state(xe, &hw_state->c10);
	else
		intel_c20pll_dump_hw_state(xe, &hw_state->c20);
}
...
static void intel_c20_pll_program(struct xe_device *xe,
				  const struct intel_crtc_state *crtc_state,
				  struct intel_encoder *encoder)
{
					 ...
					 bool lane_reversal)
{
	...
	intel_de_rmw(xe, XELPDP_PORT_BUF_CTL1(xe, encoder->port),
	...
	intel_de_rmw(xe, XELPDP_PORT_CLOCK_CTL(xe, encoder->port),
						...
						u8 lane_mask, u8 state)
{
	...
	enum port port = encoder->port;
	enum phy phy = intel_encoder_to_phy(encoder);
	i915_reg_t buf_ctl2_reg = XELPDP_PORT_BUF_CTL2(xe, port);
	int lane;

	intel_de_rmw(xe, buf_ctl2_reg,
	...
	if (intel_de_wait_custom(xe, buf_ctl2_reg,
				 intel_cx0_get_powerdown_update(lane_mask), 0,
				 XELPDP_PORT_POWERDOWN_UPDATE_TIMEOUT_US, 0, NULL))
		drm_warn(&xe->drm, "PHY %c failed to bring out of Lane reset after %dus.\n",
...
static void intel_cx0_setup_powerdown(struct intel_encoder *encoder)
{
	...
	intel_de_rmw(xe, XELPDP_PORT_BUF_CTL2(xe, port),
		     XELPDP_POWER_STATE_READY_MASK,
		     XELPDP_POWER_STATE_READY(CX0_P2_STATE_READY));
	intel_de_rmw(xe, XELPDP_PORT_BUF_CTL3(xe, port),
				     ...
				     bool lane_reversal)
{
	...
	if (intel_de_wait_custom(xe, XELPDP_PORT_BUF_CTL1(xe, port),
				 XELPDP_PORT_BUF_SOC_PHY_READY,
				 XELPDP_PORT_BUF_SOC_PHY_READY,
				 XELPDP_PORT_BUF_SOC_READY_TIMEOUT_US, 0, NULL))
		drm_warn(&xe->drm, "PHY %c failed to bring out of SOC reset after %dus.\n",
			 phy_name(phy), XELPDP_PORT_BUF_SOC_READY_TIMEOUT_US);

	intel_de_rmw(xe, XELPDP_PORT_BUF_CTL2(xe, port), lane_pipe_reset,
		     lane_pipe_reset);

	if (intel_de_wait_custom(xe, XELPDP_PORT_BUF_CTL2(xe, port),
				 lane_phy_current_status, lane_phy_current_status,
				 XELPDP_PORT_RESET_START_TIMEOUT_US, 0, NULL))
		drm_warn(&xe->drm, "PHY %c failed to bring out of Lane reset after %dus.\n",
			 phy_name(phy), XELPDP_PORT_RESET_START_TIMEOUT_US);

	intel_de_rmw(xe, XELPDP_PORT_CLOCK_CTL(xe, port),
		     intel_cx0_get_pclk_refclk_request(owned_lane_mask),
		     intel_cx0_get_pclk_refclk_request(lane_mask));

	if (intel_de_wait_custom(xe, XELPDP_PORT_CLOCK_CTL(xe, port),
				 intel_cx0_get_pclk_refclk_ack(owned_lane_mask),
				 intel_cx0_get_pclk_refclk_ack(lane_mask),
				 XELPDP_REFCLK_ENABLE_TIMEOUT_US, 0, NULL))
		drm_warn(&xe->drm, "PHY %c failed to request refclk after %dus.\n",
	...
	intel_de_rmw(xe, XELPDP_PORT_BUF_CTL2(xe, port), lane_pipe_reset, 0);

	if (intel_de_wait_for_clear(xe, XELPDP_PORT_BUF_CTL2(xe, port),
				    lane_phy_current_status,
				    XELPDP_PORT_RESET_END_TIMEOUT))
		drm_warn(&xe->drm, "PHY %c failed to bring out of Lane reset after %dms.\n",
			 phy_name(phy), XELPDP_PORT_RESET_END_TIMEOUT);
}

static void intel_cx0_program_phy_lane(struct xe_device *xe,
				...
				const struct intel_crtc_state *crtc_state)
{
	...
	if (intel_encoder_is_c10phy(encoder))
		intel_c10_pll_program(xe, crtc_state, encoder);
	else
		intel_c20_pll_program(xe, crtc_state, encoder);

	/*
	 * 6. Program the enabled and disabled owned PHY lane
	 * transmitters over message bus
	 */
	intel_cx0_program_phy_lane(xe, encoder, crtc_state->lane_count, lane_reversal);
	 ...
	 * clock frequency.
	 */
	intel_de_write(xe, DDI_CLK_VALFREQ(encoder->port),
	...
	if (intel_de_wait_custom(xe, XELPDP_PORT_CLOCK_CTL(xe, encoder->port),
				 intel_cx0_get_pclk_pll_ack(INTEL_CX0_BOTH_LANES),
				 intel_cx0_get_pclk_pll_ack(maxpclk_lane),
				 XELPDP_PCLK_PLL_ENABLE_TIMEOUT_US, 0, NULL))
		drm_warn(&xe->drm, "Port %c PLL not locked after %dus.\n",
...
int intel_mtl_tbt_calc_port_clock(struct intel_encoder *encoder)
{
	struct xe_device *xe = to_xe(encoder->base.dev);
	u32 clock;
	u32 val = intel_de_read(xe, XELPDP_PORT_CLOCK_CTL(xe, encoder->port));

	clock = REG_FIELD_GET(XELPDP_DDI_CLOCK_SELECT_MASK, val);

	drm_WARN_ON(&xe->drm, !(val & XELPDP_FORWARD_CLOCK_UNGATE));
	drm_WARN_ON(&xe->drm, !(val & XELPDP_TBT_CLOCK_REQUEST));
	drm_WARN_ON(&xe->drm, !(val & XELPDP_TBT_CLOCK_ACK));
...
static int intel_mtl_tbt_clock_select(struct xe_device *xe, int clock)
{
	switch (clock) {
	case 162000:
				     ...
				     const struct intel_crtc_state *crtc_state)
{
	 ...
	 * clock muxes, gating and SSC
	 */
	val |= XELPDP_DDI_CLOCK_SELECT(intel_mtl_tbt_clock_select(xe, crtc_state->port_clock));
	val |= XELPDP_FORWARD_CLOCK_UNGATE;
	intel_de_rmw(xe, XELPDP_PORT_CLOCK_CTL(xe, encoder->port),
		     XELPDP_DDI_CLOCK_SELECT_MASK | XELPDP_FORWARD_CLOCK_UNGATE, val);

	/* 2. Read back PORT_CLOCK_CTL REGISTER */
	val = intel_de_read(xe, XELPDP_PORT_CLOCK_CTL(xe, encoder->port));
	...
	intel_de_write(xe, XELPDP_PORT_CLOCK_CTL(xe, encoder->port), val);
	...
	if (intel_de_wait_custom(xe, XELPDP_PORT_CLOCK_CTL(xe, encoder->port),
				 ...
				 XELPDP_TBT_CLOCK_ACK,
				 100, 0, NULL))
		drm_warn(&xe->drm, "[ENCODER:%d:%s][%c] PHY PLL not locked after 100us.\n",
...
static u8 cx0_power_control_disable_val(struct intel_encoder *encoder)
{
	...
	if (IS_BATTLEMAGE(xe) && encoder->port == PORT_A)
		return CX0_P2PG_STATE_DISABLE;
...
static void intel_cx0pll_disable(struct intel_encoder *encoder)
{
	...
	intel_de_write(xe, DDI_CLK_VALFREQ(encoder->port), 0);
	...
	if (intel_de_wait_custom(xe, XELPDP_PORT_CLOCK_CTL(xe, encoder->port),
		...
		drm_warn(&xe->drm, "Port %c PLL not unlocked after %dus.\n",
...
static void intel_mtl_tbt_pll_disable(struct intel_encoder *encoder)
{
	...
	if (intel_de_wait_custom(xe, XELPDP_PORT_CLOCK_CTL(xe, encoder->port),
		...
		drm_warn(&xe->drm, "[ENCODER:%d:%s][%c] PHY PLL not unlocked after 10us.\n",
			 encoder->base.base.id, encoder->base.name, phy_name(phy));
				      ...
				      struct intel_c10pll_state *mpllb_hw_state)
{
	...
	for (i = 0; i < ARRAY_SIZE(mpllb_sw_state->pll); i++) {
		...
		I915_STATE_WARN(xe, mpllb_hw_state->pll[i] != expected,
				"[CRTC:%d:%s] mismatch in C10MPLLB: Register[%d] (expected 0x%02x, found 0x%02x)",
				crtc->base.base.id, crtc->base.name, i,
				expected, mpllb_hw_state->pll[i]);
	}

	I915_STATE_WARN(xe, mpllb_hw_state->tx != mpllb_sw_state->tx,
			"[CRTC:%d:%s] mismatch in C10MPLLB: Register TX0 (expected 0x%02x, found 0x%02x)",
			crtc->base.base.id, crtc->base.name,
			mpllb_sw_state->tx, mpllb_hw_state->tx);

	I915_STATE_WARN(xe, mpllb_hw_state->cmn != mpllb_sw_state->cmn,
				      ...
				      struct intel_c20pll_state *mpll_hw_state)
{
	...
	I915_STATE_WARN(xe, mpll_hw_state->clock != clock,
			"[CRTC:%d:%s] mismatch in C20: Register CLOCK (expected %d, found %d)",
			crtc->base.base.id, crtc->base.name,
			mpll_sw_state->clock, mpll_hw_state->clock);

	I915_STATE_WARN(xe, sw_use_mpllb != hw_use_mpllb,
	...
	if (hw_use_mpllb) {
		for (i = 0; i < ARRAY_SIZE(mpll_sw_state->mpllb); i++) {
			I915_STATE_WARN(xe, mpll_hw_state->mpllb[i] != mpll_sw_state->mpllb[i],
		...
		for (i = 0; i < ARRAY_SIZE(mpll_sw_state->mplla); i++) {
			I915_STATE_WARN(xe, mpll_hw_state->mplla[i] != mpll_sw_state->mplla[i],
	...
	for (i = 0; i < ARRAY_SIZE(mpll_sw_state->tx); i++) {
		I915_STATE_WARN(xe, mpll_hw_state->tx[i] != mpll_sw_state->tx[i],
	...
	for (i = 0; i < ARRAY_SIZE(mpll_sw_state->cmn); i++) {
		I915_STATE_WARN(xe, mpll_hw_state->cmn[i] != mpll_sw_state->cmn[i],
