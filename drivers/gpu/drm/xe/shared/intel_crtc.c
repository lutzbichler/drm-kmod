...
static void assert_vblank_disabled(struct drm_crtc *crtc)
{
	...
	if (I915_STATE_WARN(xe, drm_crtc_vblank_get(crtc) == 0,
			    "[CRTC:%d:%s] vblank assertion failure (expected off, current on)\n",
			    crtc->base.id, crtc->name))
		drm_crtc_vblank_put(crtc);
}

struct intel_crtc *intel_first_crtc(struct xe_device *xe)
{
	return to_intel_crtc(drm_crtc_from_index(&xe->drm, 0));
}

struct intel_crtc *intel_crtc_for_pipe(struct xe_device *xe,
				       enum pipe pipe)
{
...
void intel_wait_for_vblank_if_active(struct xe_device *xe,
				     enum pipe pipe)
{
...
int intel_crtc_init(struct xe_device *xe, enum pipe pipe)
{
	...
	crtc->pipe = pipe;
	crtc->num_scalers = DISPLAY_RUNTIME_INFO(xe)->num_scalers[pipe];

	if (DISPLAY_VER(xe) >= 9)
		primary = skl_universal_plane_create(xe, pipe, PLANE_1);
	else
		primary = intel_primary_plane_create(xe, pipe);
	if (IS_ERR(primary)) {
		ret = PTR_ERR(primary);
		goto fail;
	}
	...
	intel_init_fifo_underrun_reporting(xe, crtc, false);

	for_each_sprite(xe, pipe, sprite) {
		struct intel_plane *plane;

		if (DISPLAY_VER(xe) >= 9)
			plane = skl_universal_plane_create(xe, pipe, PLANE_2 + sprite);
		else
			plane = intel_sprite_plane_create(xe, pipe, sprite);
		if (IS_ERR(plane)) {
			ret = PTR_ERR(plane);
			goto fail;
		}
	...
	cursor = intel_cursor_plane_create(xe, pipe);
	if (IS_ERR(cursor)) {
		ret = PTR_ERR(cursor);
		goto fail;
	}
	...
	if (HAS_GMCH(xe)) {
		if (IS_CHERRYVIEW(xe) ||
		    IS_VALLEYVIEW(xe) || IS_G4X(xe))
			funcs = &g4x_crtc_funcs;
		else if (DISPLAY_VER(xe) == 4)
			funcs = &i965_crtc_funcs;
		else if (IS_I945GM(xe) || IS_I915GM(xe))
	...
	ret = drm_crtc_init_with_planes(&xe->drm, &crtc->base,
	...
	drm_WARN_ON(&xe->drm, drm_crtc_index(&crtc->base) != crtc->pipe);
