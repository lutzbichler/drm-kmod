...
static void
intel_dump_infoframe(struct xe_device *xe,
		     const union hdmi_infoframe *frame)
{
	if (!drm_debug_enabled(DRM_UT_KMS))
		return;

	hdmi_infoframe_log(KERN_DEBUG, xe->drm.dev, frame);
}
...
static void
ilk_dump_csc(struct xe_device *xe,
			   ...
			   const char *context)
{
	...
	p = drm_dbg_printer(&xe->drm, DRM_UT_KMS, NULL);
	...
	if (pipe_config->infoframes.enable &
		...
		intel_dump_infoframe(xe, &pipe_config->infoframes.avi);
	if (pipe_config->infoframes.enable &
	    intel_hdmi_infoframe_enable(HDMI_INFOFRAME_TYPE_SPD))
		intel_dump_infoframe(xe, &pipe_config->infoframes.spd);
	if (pipe_config->infoframes.enable &
	    intel_hdmi_infoframe_enable(HDMI_INFOFRAME_TYPE_VENDOR))
		intel_dump_infoframe(xe, &pipe_config->infoframes.hdmi);
	if (pipe_config->infoframes.enable &
	    intel_hdmi_infoframe_enable(HDMI_INFOFRAME_TYPE_DRM))
		intel_dump_infoframe(xe, &pipe_config->infoframes.drm);
	if (pipe_config->infoframes.enable &
	...
	intel_dpll_dump_hw_state(xe, &p, &pipe_config->dpll_hw_state);
	...
	drm_printf(&p, "pre csc lut: %s%d entries, post csc lut: %d entries\n",
		   ...
		   xe->display.color.glk_linear_degamma_lut ? "(linear) " : "",
	...
	if (DISPLAY_VER(xe) >= 11)
		ilk_dump_csc(xe, &p, "output csc", &pipe_config->output_csc);

	if (!HAS_GMCH(xe))
		ilk_dump_csc(xe, &p, "pipe csc", &pipe_config->csc);
