...
#ifdef I915
void bxt_port_to_phy_channel(struct xe_device *xe, enum port port,
			     enum dpio_phy *phy, enum dpio_channel *ch);
void bxt_dpio_phy_set_signal_levels(struct intel_encoder *encoder,
				    const struct intel_crtc_state *crtc_state);
void bxt_dpio_phy_init(struct xe_device *xe, enum dpio_phy phy);
void bxt_dpio_phy_uninit(struct xe_device *xe, enum dpio_phy phy);
bool bxt_dpio_phy_is_enabled(struct xe_device *xe,
			     enum dpio_phy phy);
bool bxt_dpio_phy_verify_state(struct xe_device *xe,
			       enum dpio_phy phy);
...
void vlv_phy_reset_lanes(struct intel_encoder *encoder,
			 const struct intel_crtc_state *old_crtc_state);
#else
static inline void bxt_port_to_phy_channel(struct xe_device *xe, enum port port,
					   enum dpio_phy *phy, enum dpio_channel *ch)
{
}
...
static inline void bxt_dpio_phy_init(struct xe_device *xe, enum dpio_phy phy)
{
}
static inline void bxt_dpio_phy_uninit(struct xe_device *xe, enum dpio_phy phy)
{
}
static inline bool bxt_dpio_phy_is_enabled(struct xe_device *xe,
					   enum dpio_phy phy)
{
	return false;
}
static inline bool bxt_dpio_phy_verify_state(struct xe_device *xe,
					     enum dpio_phy phy)
{
	return true;
}
