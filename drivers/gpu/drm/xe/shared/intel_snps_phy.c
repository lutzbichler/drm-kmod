...
void intel_snps_phy_wait_for_calibration(struct xe_device *xe)
{
	...
	for_each_phy_masked(phy, ~0) {
		 ...
		 * output later.
		 */
		if (intel_de_wait_for_clear(xe, DG2_PHY_MISC(phy),
					   ...
					   bool enable)
{
	...
	val = REG_FIELD_PREP(SNPS_PHY_TX_REQ_LN_DIS_PWR_STATE_PSR,
			     enable ? 2 : 3);
	intel_de_rmw(xe, SNPS_PHY_TX_REQ(phy),
				      ...
				      const struct intel_crtc_state *crtc_state)
{
	...
	for (ln = 0; ln < 4; ln++) {
		...
		intel_de_write(xe, SNPS_PHY_TX_EQ(ln, phy), val);
	}
}
			   ...
			   struct intel_encoder *encoder)
{
	...
	if (intel_crtc_has_type(crtc_state, INTEL_OUTPUT_HDMI)) {
		if (intel_snps_phy_check_hdmi_link_rate(crtc_state->port_clock)
			 ...
			 * license.
			 */
			drm_dbg_kms(&xe->drm, "Can't support HDMI link rate %d\n",
			...
			const struct intel_crtc_state *crtc_state)
{
	 ...
	 * frequency.
	 */
	intel_de_write(xe, SNPS_PHY_MPLLB_CP(phy), pll_state->mpllb_cp);
	intel_de_write(xe, SNPS_PHY_MPLLB_DIV(phy), pll_state->mpllb_div);
	intel_de_write(xe, SNPS_PHY_MPLLB_DIV2(phy), pll_state->mpllb_div2);
	intel_de_write(xe, SNPS_PHY_MPLLB_SSCEN(phy), pll_state->mpllb_sscen);
	intel_de_write(xe, SNPS_PHY_MPLLB_SSCSTEP(phy), pll_state->mpllb_sscstep);
	intel_de_write(xe, SNPS_PHY_MPLLB_FRACN1(phy), pll_state->mpllb_fracn1);
	intel_de_write(xe, SNPS_PHY_MPLLB_FRACN2(phy), pll_state->mpllb_fracn2);
	 ...
	 * divider values.
	 */
	intel_de_write(xe, SNPS_PHY_MPLLB_DIV(phy),
	 ...
	 */
	if (intel_de_wait_for_set(xe, enable_reg, PLL_LOCK, 5))
		drm_dbg_kms(&xe->drm, "Port %c PLL not locked\n", phy_name(phy));
...
void intel_mpllb_disable(struct intel_encoder *encoder)
{
	 ...
	 */
	intel_de_rmw(xe, SNPS_PHY_MPLLB_DIV(phy), SNPS_PHY_MPLLB_FORCE_EN, 0);

	/*
	 * 5. Software polls DPLL_ENABLE [PLL Lock] for PHY acknowledgment
	 * (dp_txX_ack) that the new transmitter setting request is completed.
	 */
	if (intel_de_wait_for_clear(xe, enable_reg, PLL_LOCK, 5))
		drm_err(&xe->drm, "Port %c PLL not locked\n", phy_name(phy));
				  ...
				  struct intel_mpllb_state *pll_state)
{
	...
	pll_state->mpllb_cp = intel_de_read(xe, SNPS_PHY_MPLLB_CP(phy));
	pll_state->mpllb_div = intel_de_read(xe, SNPS_PHY_MPLLB_DIV(phy));
	pll_state->mpllb_div2 = intel_de_read(xe, SNPS_PHY_MPLLB_DIV2(phy));
	pll_state->mpllb_sscen = intel_de_read(xe, SNPS_PHY_MPLLB_SSCEN(phy));
	pll_state->mpllb_sscstep = intel_de_read(xe, SNPS_PHY_MPLLB_SSCSTEP(phy));
	pll_state->mpllb_fracn1 = intel_de_read(xe, SNPS_PHY_MPLLB_FRACN1(phy));
	pll_state->mpllb_fracn2 = intel_de_read(xe, SNPS_PHY_MPLLB_FRACN2(phy));
	 ...
	 */
	pll_state->ref_control = intel_de_read(xe, SNPS_PHY_REF_CONTROL(phy)) &
...
#define MPLLB_CHECK(__name)						\
	I915_STATE_WARN(xe, mpllb_sw_state->__name != mpllb_hw_state.__name, \
