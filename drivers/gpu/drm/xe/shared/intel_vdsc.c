...
bool intel_dsc_source_support(const struct intel_crtc_state *crtc_state)
{
	...
	if (DISPLAY_VER(xe) == 11 && cpu_transcoder == TRANSCODER_A)
		return false;
...
static bool is_pipe_dsc(struct intel_crtc *crtc, enum transcoder cpu_transcoder)
{
	...
	drm_WARN_ON(&xe->drm, crtc->pipe == PIPE_A);
...
int intel_dsc_compute_params(struct intel_crtc_state *pipe_config)
{
	...
	if (err) {
		drm_dbg_kms(&xe->drm, "Slice dimension requirements not met\n");
		return err;
	}
...
intel_dsc_power_domain(struct intel_crtc *crtc, enum transcoder cpu_transcoder)
{
	 ...
	 */
	if (DISPLAY_VER(xe) == 12 && !IS_ROCKETLAKE(xe) && pipe == PIPE_A)
				...
				int pps, u32 pps_val)
{
	...
	drm_WARN_ON_ONCE(&xe->drm, dsc_reg_num < vdsc_per_pipe);

	intel_dsc_get_pps_reg(crtc_state, pps, dsc_reg, dsc_reg_num);

	for (i = 0; i < dsc_reg_num; i++)
		intel_de_write(xe, dsc_reg[i], pps_val);
}
...
static void intel_dsc_pps_configure(const struct intel_crtc_state *crtc_state)
{
	...
	if (!is_pipe_dsc(crtc, cpu_transcoder)) {
		intel_de_write(xe, DSCA_RC_BUF_THRESH_0,
			       rc_buf_thresh_dword[0]);
		intel_de_write(xe, DSCA_RC_BUF_THRESH_0_UDW,
			       rc_buf_thresh_dword[1]);
		intel_de_write(xe, DSCA_RC_BUF_THRESH_1,
			       rc_buf_thresh_dword[2]);
		intel_de_write(xe, DSCA_RC_BUF_THRESH_1_UDW,
			       rc_buf_thresh_dword[3]);
		if (vdsc_instances_per_pipe > 1) {
			intel_de_write(xe, DSCC_RC_BUF_THRESH_0,
				       rc_buf_thresh_dword[0]);
			intel_de_write(xe, DSCC_RC_BUF_THRESH_0_UDW,
				       rc_buf_thresh_dword[1]);
			intel_de_write(xe, DSCC_RC_BUF_THRESH_1,
				       rc_buf_thresh_dword[2]);
			intel_de_write(xe, DSCC_RC_BUF_THRESH_1_UDW,
				       rc_buf_thresh_dword[3]);
		}
	} else {
		intel_de_write(xe, ICL_DSC0_RC_BUF_THRESH_0(pipe),
			       rc_buf_thresh_dword[0]);
		intel_de_write(xe, ICL_DSC0_RC_BUF_THRESH_0_UDW(pipe),
			       rc_buf_thresh_dword[1]);
		intel_de_write(xe, ICL_DSC0_RC_BUF_THRESH_1(pipe),
			       rc_buf_thresh_dword[2]);
		intel_de_write(xe, ICL_DSC0_RC_BUF_THRESH_1_UDW(pipe),
			       rc_buf_thresh_dword[3]);
		if (vdsc_instances_per_pipe > 1) {
			intel_de_write(xe,
	...
	if (!is_pipe_dsc(crtc, cpu_transcoder)) {
		intel_de_write(xe, DSCA_RC_RANGE_PARAMETERS_0,
			       rc_range_params_dword[0]);
		intel_de_write(xe, DSCA_RC_RANGE_PARAMETERS_0_UDW,
			       rc_range_params_dword[1]);
		intel_de_write(xe, DSCA_RC_RANGE_PARAMETERS_1,
			       rc_range_params_dword[2]);
		intel_de_write(xe, DSCA_RC_RANGE_PARAMETERS_1_UDW,
			       rc_range_params_dword[3]);
		intel_de_write(xe, DSCA_RC_RANGE_PARAMETERS_2,
			       rc_range_params_dword[4]);
		intel_de_write(xe, DSCA_RC_RANGE_PARAMETERS_2_UDW,
			       rc_range_params_dword[5]);
		intel_de_write(xe, DSCA_RC_RANGE_PARAMETERS_3,
			       rc_range_params_dword[6]);
		intel_de_write(xe, DSCA_RC_RANGE_PARAMETERS_3_UDW,
			       rc_range_params_dword[7]);
		if (vdsc_instances_per_pipe > 1) {
			intel_de_write(xe, DSCC_RC_RANGE_PARAMETERS_0,
				       rc_range_params_dword[0]);
			intel_de_write(xe,
				       DSCC_RC_RANGE_PARAMETERS_0_UDW,
				       rc_range_params_dword[1]);
			intel_de_write(xe, DSCC_RC_RANGE_PARAMETERS_1,
				       rc_range_params_dword[2]);
			intel_de_write(xe,
				       DSCC_RC_RANGE_PARAMETERS_1_UDW,
				       rc_range_params_dword[3]);
			intel_de_write(xe, DSCC_RC_RANGE_PARAMETERS_2,
				       rc_range_params_dword[4]);
			intel_de_write(xe,
				       DSCC_RC_RANGE_PARAMETERS_2_UDW,
				       rc_range_params_dword[5]);
			intel_de_write(xe, DSCC_RC_RANGE_PARAMETERS_3,
				       rc_range_params_dword[6]);
			intel_de_write(xe,
		...
		intel_de_write(xe, ICL_DSC0_RC_RANGE_PARAMETERS_0(pipe),
			       rc_range_params_dword[0]);
		intel_de_write(xe,
			       ICL_DSC0_RC_RANGE_PARAMETERS_0_UDW(pipe),
			       rc_range_params_dword[1]);
		intel_de_write(xe, ICL_DSC0_RC_RANGE_PARAMETERS_1(pipe),
			       rc_range_params_dword[2]);
		intel_de_write(xe,
			       ICL_DSC0_RC_RANGE_PARAMETERS_1_UDW(pipe),
			       rc_range_params_dword[3]);
		intel_de_write(xe, ICL_DSC0_RC_RANGE_PARAMETERS_2(pipe),
			       rc_range_params_dword[4]);
		intel_de_write(xe,
			       ICL_DSC0_RC_RANGE_PARAMETERS_2_UDW(pipe),
			       rc_range_params_dword[5]);
		intel_de_write(xe, ICL_DSC0_RC_RANGE_PARAMETERS_3(pipe),
			       rc_range_params_dword[6]);
		intel_de_write(xe,
...
void intel_uncompressed_joiner_enable(const struct intel_crtc_state *crtc_state)
{
	...
	if (crtc_state->joiner_pipes && !crtc_state->dsc.compression_enable) {
		...
		intel_de_write(xe, dss_ctl1_reg(crtc, crtc_state->cpu_transcoder), dss_ctl1_val);
	}
}
...
void intel_dsc_enable(const struct intel_crtc_state *crtc_state)
{
	...
	intel_de_write(xe, dss_ctl1_reg(crtc, crtc_state->cpu_transcoder), dss_ctl1_val);
	intel_de_write(xe, dss_ctl2_reg(crtc, crtc_state->cpu_transcoder), dss_ctl2_val);
}
...
void intel_dsc_disable(const struct intel_crtc_state *old_crtc_state)
{
	...
	if (old_crtc_state->dsc.compression_enable ||
		...
		intel_de_write(xe, dss_ctl1_reg(crtc, old_crtc_state->cpu_transcoder), 0);
		intel_de_write(xe, dss_ctl2_reg(crtc, old_crtc_state->cpu_transcoder), 0);
	}
}
			      ...
			      bool *all_equal)
{
	...
	*all_equal = true;
	...
	val = intel_de_read(xe, dsc_reg[0]);

	for (i = 1; i < dsc_reg_num; i++) {
		if (intel_de_read(xe, dsc_reg[i]) != val) {
			*all_equal = false;
			break;
		}
	}
...
void intel_dsc_get_config(struct intel_crtc_state *crtc_state)
{
	...
	dss_ctl1 = intel_de_read(xe, dss_ctl1_reg(crtc, cpu_transcoder));
	dss_ctl2 = intel_de_read(xe, dss_ctl2_reg(crtc, cpu_transcoder));
