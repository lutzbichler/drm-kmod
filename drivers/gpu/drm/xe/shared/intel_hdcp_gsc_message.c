				...
				struct hdcp2_ake_init *ake_data)
{
	...
	byte = intel_hdcp_gsc_msg_send(xe, (u8 *)&session_init_in,
				       sizeof(session_init_in),
				       (u8 *)&session_init_out,
				       sizeof(session_init_out));
	if (byte < 0) {
		drm_dbg_kms(&xe->drm, "intel_hdcp_gsc_msg_send failed. %zd\n", byte);
		return byte;
	}

	if (session_init_out.header.status != FW_HDCP_STATUS_SUCCESS) {
		drm_dbg_kms(&xe->drm, "FW cmd 0x%08X Failed. Status: 0x%X\n",
					       ...
					       size_t *msg_sz)
{
	...
	byte = intel_hdcp_gsc_msg_send(xe, (u8 *)&verify_rxcert_in,
			     ...
			     struct hdcp2_ake_send_hprime *rx_hprime)
{
	...
	byte = intel_hdcp_gsc_msg_send(xe, (u8 *)&send_hprime_in,
				  ...
				  struct hdcp2_ake_send_pairing_info *pairing_info)
{
	...
	byte = intel_hdcp_gsc_msg_send(xe, (u8 *)&pairing_info_in,
				       ...
				       struct hdcp2_lc_init *lc_init_data)
{
	...
	byte = intel_hdcp_gsc_msg_send(xe, (u8 *)&lc_init_in, sizeof(lc_init_in),
			     ...
			     struct hdcp2_lc_send_lprime *rx_lprime)
{
	...
	byte = intel_hdcp_gsc_msg_send(xe, (u8 *)&verify_lprime_in,
				   ...
				   struct hdcp2_ske_send_eks *ske_data)
{
	...
	byte = intel_hdcp_gsc_msg_send(xe, (u8 *)&get_skey_in, sizeof(get_skey_in),
					       ...
					       *rep_send_ack)
{
	...
	byte = intel_hdcp_gsc_msg_send(xe, (u8 *)&verify_repeater_in,
				 ...
				 struct hdcp2_rep_stream_ready *stream_ready)
{
	...
	byte = intel_hdcp_gsc_msg_send(xe, (u8 *)verify_mprime_in, cmd_size,
					 ...
					 struct hdcp_port_data *data)
{
	...
	byte = intel_hdcp_gsc_msg_send(xe, (u8 *)&enable_auth_in,
...
intel_hdcp_gsc_close_session(struct device *dev, struct hdcp_port_data *data)
{
	...
	byte = intel_hdcp_gsc_msg_send(xe, (u8 *)&session_close_in,
	...
	if (session_close_out.header.status != FW_HDCP_STATUS_SUCCESS) {
		drm_dbg_kms(&xe->drm, "Session Close Failed. status: 0x%X\n",
