...
static int i9xx_pll_refclk(const struct intel_crtc_state *crtc_state)
{
	...
	if ((hw_state->dpll & PLL_REF_INPUT_MASK) == PLLB_REF_INPUT_SPREADSPECTRUMIN)
		return xe->display.vbt.lvds_ssc_freq;
			    ...
			    struct intel_dpll_hw_state *dpll_hw_state)
{
	...
	if (DISPLAY_VER(xe) >= 4) {
		...
		if (IS_CHERRYVIEW(xe) && crtc->pipe != PIPE_A)
			tmp = xe->display.state.chv_dpll_md[crtc->pipe];
		else
			tmp = intel_de_read(xe,
					    DPLL_MD(xe, crtc->pipe));

		hw_state->dpll_md = tmp;
	}

	hw_state->dpll = intel_de_read(xe, DPLL(xe, crtc->pipe));

	if (!IS_VALLEYVIEW(xe) && !IS_CHERRYVIEW(xe)) {
		hw_state->fp0 = intel_de_read(xe, FP0(crtc->pipe));
		hw_state->fp1 = intel_de_read(xe, FP1(crtc->pipe));
	} else {
...
void i9xx_crtc_clock_get(struct intel_crtc_state *crtc_state)
{
	...
	if (DISPLAY_VER(xe) != 2) {
		...
		if (IS_I85X(xe) &&
		    intel_lvds_port_enabled(xe, LVDS, &lvds_pipe) &&
		    lvds_pipe == crtc->pipe) {
			u32 lvds = intel_de_read(xe, LVDS);
...
void vlv_crtc_clock_get(struct intel_crtc_state *crtc_state)
{
	...
	vlv_dpio_get(xe);
	tmp = vlv_dpio_read(xe, phy, VLV_PLL_DW3(ch));
	vlv_dpio_put(xe);
...
void chv_crtc_clock_get(struct intel_crtc_state *crtc_state)
{
	...
	vlv_dpio_get(xe);
	cmn_dw13 = vlv_dpio_read(xe, phy, CHV_CMN_DW13(ch));
	pll_dw0 = vlv_dpio_read(xe, phy, CHV_PLL_DW0(ch));
	pll_dw1 = vlv_dpio_read(xe, phy, CHV_PLL_DW1(ch));
	pll_dw2 = vlv_dpio_read(xe, phy, CHV_PLL_DW2(ch));
	pll_dw3 = vlv_dpio_read(xe, phy, CHV_PLL_DW3(ch));
	vlv_dpio_put(xe);
 ...
 */
static bool intel_pll_is_valid(struct xe_device *xe,
		...
		for (clock.m2 = limit->m2.min;
			...
			for (clock.n = limit->n.min;
				...
				for (clock.p1 = limit->p1.min;
					...
					if (!intel_pll_is_valid(to_xe(dev),
								limit,
								&clock))
						continue;
					if (match_clock &&
		   ...
		   struct dpll *best_clock)
{
	...
	for (clock.p1 = limit->p1.max; clock.p1 >= limit->p1.min; clock.p1--) {
		for (clock.p2 = limit->p2.p2_fast;
			...
			if (!intel_pll_is_valid(to_xe(dev), limit, &clock))
				continue;
...
static int ilk_fb_cb_factor(const struct intel_crtc_state *crtc_state)
{
	...
	if (intel_crtc_has_type(crtc_state, INTEL_OUTPUT_LVDS) &&
	    ((intel_panel_use_ssc(xe) && xe->display.vbt.lvds_ssc_freq == 100000) ||
	     (HAS_PCH_IBX(xe) && intel_is_dual_link_lvds(xe))))
		return 25;
	...
	if (intel_crtc_has_type(crtc_state, INTEL_OUTPUT_LVDS)) {
		if (intel_panel_use_ssc(xe)) {
			drm_dbg_kms(&xe->drm,
				    ...
				    xe->display.vbt.lvds_ssc_freq);
			refclk = xe->display.vbt.lvds_ssc_freq;
		}
				  ...
				  struct intel_crtc *crtc)
{
	...
	drm_WARN_ON(&xe->drm, !intel_crtc_needs_modeset(crtc_state));
	...
	ret = xe->display.funcs.dpll->crtc_compute_clock(state, crtc);
	if (ret) {
		drm_dbg_kms(&xe->drm, "[CRTC:%d:%s] Couldn't calculate DPLL settings\n",
				    ...
				    struct intel_crtc *crtc)
{
	...
	drm_WARN_ON(&xe->drm, !intel_crtc_needs_modeset(crtc_state));
	drm_WARN_ON(&xe->drm, !crtc_state->hw.enable && crtc_state->shared_dpll);

	if (!crtc_state->hw.enable || crtc_state->shared_dpll)
		return 0;

	if (!xe->display.funcs.dpll->crtc_get_shared_dpll)
		return 0;

	ret = xe->display.funcs.dpll->crtc_get_shared_dpll(state, crtc);
	if (ret) {
		drm_dbg_kms(&xe->drm, "[CRTC:%d:%s] Couldn't get a shared DPLL\n",
			    crtc->base.base.id, crtc->base.name);
		return ret;
	}
...
void
intel_dpll_init_clock_hook(struct xe_device *xe)
{
	if (DISPLAY_VER(xe) >= 14)
		xe->display.funcs.dpll = &mtl_dpll_funcs;
	else if (IS_DG2(xe))
		xe->display.funcs.dpll = &dg2_dpll_funcs;
	else if (DISPLAY_VER(xe) >= 9 || HAS_DDI(xe))
		xe->display.funcs.dpll = &hsw_dpll_funcs;
	else if (HAS_PCH_SPLIT(xe))
		xe->display.funcs.dpll = &ilk_dpll_funcs;
	else if (IS_CHERRYVIEW(xe))
		xe->display.funcs.dpll = &chv_dpll_funcs;
	else if (IS_VALLEYVIEW(xe))
		xe->display.funcs.dpll = &vlv_dpll_funcs;
	else if (IS_G4X(xe))
		xe->display.funcs.dpll = &g4x_dpll_funcs;
	else if (IS_PINEVIEW(xe))
		xe->display.funcs.dpll = &pnv_dpll_funcs;
	else if (DISPLAY_VER(xe) != 2)
		xe->display.funcs.dpll = &i9xx_dpll_funcs;
	else
		xe->display.funcs.dpll = &i8xx_dpll_funcs;
}

static bool i9xx_has_pps(struct xe_device *xe)
{
	if (IS_I830(xe))
		return false;

	return IS_PINEVIEW(xe) || IS_MOBILE(xe);
}
...
void i9xx_enable_pll(const struct intel_crtc_state *crtc_state)
{
	...
	intel_de_write(xe, FP0(pipe), hw_state->fp0);
	intel_de_write(xe, FP1(pipe), hw_state->fp1);
	...
	intel_de_write(xe, DPLL(xe, pipe), hw_state->dpll);
	...
	if (DISPLAY_VER(xe) >= 4) {
		intel_de_write(xe, DPLL_MD(xe, pipe),
			       hw_state->dpll_md);
	} else {
...
static void vlv_pllb_recal_opamp(struct xe_device *xe,
				 enum dpio_phy phy, enum dpio_channel ch)
{
	 ...
	 */
	tmp = vlv_dpio_read(xe, phy, VLV_PLL_DW17(ch));
	tmp &= 0xffffff00;
	tmp |= 0x00000030;
	vlv_dpio_write(xe, phy, VLV_PLL_DW17(ch), tmp);

	tmp = vlv_dpio_read(xe, phy, VLV_REF_DW11);
	tmp &= 0x00ffffff;
	tmp |= 0x8c000000;
	vlv_dpio_write(xe, phy, VLV_REF_DW11, tmp);
...
static void vlv_prepare_pll(const struct intel_crtc_state *crtc_state)
{
	...
	if (pipe == PIPE_B)
		vlv_pllb_recal_opamp(xe, phy, ch);

	/* Set up Tx target for periodic Rcomp update */
	vlv_dpio_write(xe, phy, VLV_PCS_DW17_BCAST, 0x0100000f);

	/* Disable target IRef on PLL */
	tmp = vlv_dpio_read(xe, phy, VLV_PLL_DW16(ch));
	tmp &= 0x00ffffff;
	vlv_dpio_write(xe, phy, VLV_PLL_DW16(ch), tmp);

	/* Disable fast lock */
	vlv_dpio_write(xe, phy, VLV_CMN_DW0, 0x610);
	...
	vlv_dpio_write(xe, phy, VLV_PLL_DW3(ch), tmp);
	...
	if (crtc_state->port_clock == 162000 ||
	    intel_crtc_has_type(crtc_state, INTEL_OUTPUT_ANALOG) ||
	    intel_crtc_has_type(crtc_state, INTEL_OUTPUT_HDMI))
		vlv_dpio_write(xe, phy, VLV_PLL_DW18(ch),
				 0x009f0003);
	else
		vlv_dpio_write(xe, phy, VLV_PLL_DW18(ch),
				 0x00d0000f);
	...
	if (intel_crtc_has_dp_encoder(crtc_state)) {
		/* Use SSC source */
		if (pipe == PIPE_A)
			vlv_dpio_write(xe, phy, VLV_PLL_DW5(ch),
					 0x0df40000);
		else
			vlv_dpio_write(xe, phy, VLV_PLL_DW5(ch),
					 0x0df70000);
	...
	coreclk = vlv_dpio_read(xe, phy, VLV_PLL_DW7(ch));
	coreclk = (coreclk & 0x0000ff00) | 0x01c00000;
	if (intel_crtc_has_dp_encoder(crtc_state))
		coreclk |= 0x01000000;
	vlv_dpio_write(xe, phy, VLV_PLL_DW7(ch), coreclk);

	vlv_dpio_write(xe, phy, VLV_PLL_DW19(ch), 0x87871000);
...
static void _vlv_enable_pll(const struct intel_crtc_state *crtc_state)
{
	...
	if (intel_de_wait_for_set(xe, DPLL(xe, pipe), DPLL_LOCK_VLV, 1))
		drm_err(&xe->drm, "DPLL %d failed to lock\n", pipe);
}
...
void vlv_enable_pll(const struct intel_crtc_state *crtc_state)
{
	...
	intel_de_write(xe, DPLL_MD(xe, pipe), hw_state->dpll_md);
	intel_de_posting_read(xe, DPLL_MD(xe, pipe));
}
...
static void chv_prepare_pll(const struct intel_crtc_state *crtc_state)
{
	...
	/* p1 and p2 divider */
	vlv_dpio_write(xe, phy, CHV_CMN_DW13(ch),
		       DPIO_CHV_S1_DIV(5) |
	...
	vlv_dpio_write(xe, phy, CHV_PLL_DW6(ch), loopfilter);

	tmp = vlv_dpio_read(xe, phy, CHV_PLL_DW8(ch));
	tmp &= ~DPIO_CHV_TDC_TARGET_CNT_MASK;
	tmp |= DPIO_CHV_TDC_TARGET_CNT(tribuf_calcntr);
	vlv_dpio_write(xe, phy, CHV_PLL_DW8(ch), tmp);

	/* AFC Recal */
	vlv_dpio_write(xe, phy, CHV_CMN_DW14(ch),
		       vlv_dpio_read(xe, phy, CHV_CMN_DW14(ch)) |
		       DPIO_AFC_RECAL);
...
static void _chv_enable_pll(const struct intel_crtc_state *crtc_state)
{
	...
	tmp = vlv_dpio_read(xe, phy, CHV_CMN_DW14(ch));
	tmp |= DPIO_DCLKP_EN;
	vlv_dpio_write(xe, phy, CHV_CMN_DW14(ch), tmp);
	...
	/* Check PLL is locked */
	if (intel_de_wait_for_set(xe, DPLL(xe, pipe), DPLL_LOCK_VLV, 1))
		drm_err(&xe->drm, "PLL %d failed to lock\n", pipe);
}
...
void chv_enable_pll(const struct intel_crtc_state *crtc_state)
{
	...
	if (pipe != PIPE_A) {
		 ...
		 */
		intel_de_write(xe, CBR4_VLV, CBR_DPLLBMD_PIPE(pipe));
		intel_de_write(xe, DPLL_MD(xe, PIPE_B),
			       hw_state->dpll_md);
		intel_de_write(xe, CBR4_VLV, 0);
		xe->display.state.chv_dpll_md[pipe] = hw_state->dpll_md;
		 ...
		 */
		drm_WARN_ON(&xe->drm,
			    (intel_de_read(xe, DPLL(xe, PIPE_B)) &
			     DPLL_VGA_MODE_DIS) == 0);
	} else {
		intel_de_write(xe, DPLL_MD(xe, pipe),
			       hw_state->dpll_md);
...
void vlv_disable_pll(struct xe_device *xe, enum pipe pipe)
{
	...
	assert_transcoder_disabled(xe, (enum transcoder)pipe);
...
void chv_disable_pll(struct xe_device *xe, enum pipe pipe)
{
	...
	val = vlv_dpio_read(xe, phy, CHV_CMN_DW14(ch));
	val &= ~DPIO_DCLKP_EN;
	vlv_dpio_write(xe, phy, CHV_CMN_DW14(ch), val);
...
static void assert_pll(struct xe_device *xe,
		       enum pipe pipe, bool state)
{
	bool cur_state;

	cur_state = intel_de_read(xe, DPLL(xe, pipe)) & DPLL_VCO_ENABLE;
