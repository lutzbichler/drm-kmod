...
static u8 icl_nv12_y_plane_mask(struct xe_device *xe)
{
			 ...
			 enum plane_id plane_id)
{
	return DISPLAY_VER(xe) >= 11 &&
		icl_nv12_y_plane_mask(xe) & BIT(plane_id);
}
				   ...
				   int color_plane)
{
	...
	case I915_FORMAT_MOD_4_TILED:
		 ...
		 */
		if (IS_ALDERLAKE_P(xe) && HAS_ASYNC_FLIPS(xe))
			return mult * 16 * 1024;
		return mult * 4 * 1024;
	case I915_FORMAT_MOD_Y_TILED_GEN12_MC_CCS:
		      ...
		      const struct intel_plane_state *plane_state)
{
	...
	intel_de_write_fw(xe, PLANE_INPUT_CSC_COEFF(pipe, plane_id, 0),
			  ROFF(csc[0]) | GOFF(csc[1]));
	intel_de_write_fw(xe, PLANE_INPUT_CSC_COEFF(pipe, plane_id, 1),
			  BOFF(csc[2]));
	intel_de_write_fw(xe, PLANE_INPUT_CSC_COEFF(pipe, plane_id, 2),
			  ROFF(csc[3]) | GOFF(csc[4]));
	intel_de_write_fw(xe, PLANE_INPUT_CSC_COEFF(pipe, plane_id, 3),
			  BOFF(csc[5]));
	intel_de_write_fw(xe, PLANE_INPUT_CSC_COEFF(pipe, plane_id, 4),
			  ROFF(csc[6]) | GOFF(csc[7]));
	intel_de_write_fw(xe, PLANE_INPUT_CSC_COEFF(pipe, plane_id, 5),
			  BOFF(csc[8]));

	intel_de_write_fw(xe, PLANE_INPUT_CSC_PREOFF(pipe, plane_id, 0),
			  PREOFF_YUV_TO_RGB_HI);
	intel_de_write_fw(xe, PLANE_INPUT_CSC_PREOFF(pipe, plane_id, 1),
			  PREOFF_YUV_TO_RGB_ME);
	intel_de_write_fw(xe, PLANE_INPUT_CSC_PREOFF(pipe, plane_id, 2),
			       ...
			       const struct intel_crtc_state *crtc_state)
{
	...
	for (level = 0; level < xe->display.wm.num_levels; level++)
		intel_de_write_fw(xe, PLANE_WM(pipe, plane_id, level),
				  skl_plane_wm_reg_val(skl_plane_wm_level(pipe_wm, plane_id, level)));

	intel_de_write_fw(xe, PLANE_WM_TRANS(pipe, plane_id),
			  skl_plane_wm_reg_val(skl_plane_trans_wm(pipe_wm, plane_id)));

	if (HAS_HW_SAGV_WM(xe)) {
		const struct skl_plane_wm *wm = &pipe_wm->planes[plane_id];

		intel_de_write_fw(xe, PLANE_WM_SAGV(pipe, plane_id),
				  skl_plane_wm_reg_val(&wm->sagv.wm0));
		intel_de_write_fw(xe, PLANE_WM_SAGV_TRANS(pipe, plane_id),
				  skl_plane_wm_reg_val(&wm->sagv.trans_wm));
	}

	intel_de_write_fw(xe, PLANE_BUF_CFG(pipe, plane_id),
			  skl_plane_ddb_reg_val(ddb));

	if (DISPLAY_VER(xe) < 11)
		intel_de_write_fw(xe, PLANE_NV12_BUF_CFG(pipe, plane_id),
		      ...
		      const struct intel_crtc_state *crtc_state)
{
	...
	intel_de_write_fw(xe, PLANE_CTL(pipe, plane_id), 0);
	intel_de_write_fw(xe, PLANE_SURF(pipe, plane_id), 0);
}
					    ...
					    const struct intel_crtc_state *crtc_state)
{
	...
	intel_de_write_fw(xe, SEL_FETCH_PLANE_CTL(pipe, plane->id), 0);
}
		      ...
		      const struct intel_crtc_state *crtc_state)
{
	...
	if (icl_is_hdr_plane(xe, plane_id))
		intel_de_write_fw(xe, PLANE_CUS_CTL(pipe, plane_id), 0);
		       ...
		       enum pipe *pipe)
{
	...
	ret = intel_de_read(xe, PLANE_CTL(plane->pipe, plane_id)) & PLANE_CTL_ENABLE;
...
static u32 skl_plane_ctl_rotate(unsigned int rotate)
{
	 ...
	 * while xe HW rotation is clockwise, thats why this swapping.
	 */
			       ...
			       const struct intel_plane_state *plane_state)
{
	...
	if (fb->format->is_yuv && !icl_is_hdr_plane(xe, plane->id)) {
			    ...
			    int color_plane)
{
	...
	if (intel_fb_uses_dpt(fb)) {
		 ...
		 */
		drm_WARN_ON(&xe->drm, plane_state->dpt_vma &&
			    intel_dpt_offset(plane_state->dpt_vma));
		drm_WARN_ON(&xe->drm, offset & 0x1fffff);
		return offset >> 9;
	} else {
		drm_WARN_ON(&xe->drm, offset & 0xfff);
		return offset;
	}
}
...
static void icl_plane_csc_load_black(struct intel_plane *plane)
{
	...
	intel_de_write_fw(xe, PLANE_CSC_COEFF(pipe, plane_id, 0), 0);
	intel_de_write_fw(xe, PLANE_CSC_COEFF(pipe, plane_id, 1), 0);

	intel_de_write_fw(xe, PLANE_CSC_COEFF(pipe, plane_id, 2), 0);
	intel_de_write_fw(xe, PLANE_CSC_COEFF(pipe, plane_id, 3), 0);

	intel_de_write_fw(xe, PLANE_CSC_COEFF(pipe, plane_id, 4), 0);
	intel_de_write_fw(xe, PLANE_CSC_COEFF(pipe, plane_id, 5), 0);

	intel_de_write_fw(xe, PLANE_CSC_PREOFF(pipe, plane_id, 0), 0);
	intel_de_write_fw(xe, PLANE_CSC_PREOFF(pipe, plane_id, 1), 0);
	intel_de_write_fw(xe, PLANE_CSC_PREOFF(pipe, plane_id, 2), 0);

	intel_de_write_fw(xe, PLANE_CSC_POSTOFF(pipe, plane_id, 0), 0);
	intel_de_write_fw(xe, PLANE_CSC_POSTOFF(pipe, plane_id, 1), 0);
	intel_de_write_fw(xe, PLANE_CSC_POSTOFF(pipe, plane_id, 2), 0);
}
		       ...
		       const struct intel_plane_state *plane_state)
{
	...
	intel_de_write_fw(xe, PLANE_STRIDE(pipe, plane_id),
			  PLANE_STRIDE_(stride));
	intel_de_write_fw(xe, PLANE_POS(pipe, plane_id),
			  PLANE_POS_Y(crtc_y) | PLANE_POS_X(crtc_x));
	intel_de_write_fw(xe, PLANE_SIZE(pipe, plane_id),
		     ...
		     const struct intel_plane_state *plane_state)
{
	...
	intel_de_write_fw(xe, PLANE_KEYVAL(pipe, plane_id), skl_plane_keyval(plane_state));
	intel_de_write_fw(xe, PLANE_KEYMSK(pipe, plane_id), skl_plane_keymsk(plane_state));
	intel_de_write_fw(xe, PLANE_KEYMAX(pipe, plane_id), skl_plane_keymax(plane_state));

	intel_de_write_fw(xe, PLANE_OFFSET(pipe, plane_id),
			  PLANE_OFFSET_Y(y) | PLANE_OFFSET_X(x));

	intel_de_write_fw(xe, PLANE_AUX_DIST(pipe, plane_id),
			  skl_plane_aux_dist(plane_state, 0));

	intel_de_write_fw(xe, PLANE_AUX_OFFSET(pipe, plane_id),
			  PLANE_OFFSET_Y(plane_state->view.color_plane[1].y) |
			  PLANE_OFFSET_X(plane_state->view.color_plane[1].x));

	if (DISPLAY_VER(xe) >= 10)
		intel_de_write_fw(xe, PLANE_COLOR_CTL(pipe, plane_id), plane_color_ctl);
	 ...
	 */
	intel_de_write_fw(xe, PLANE_CTL(pipe, plane_id), plane_ctl);
	intel_de_write_fw(xe, PLANE_SURF(pipe, plane_id),
					     ...
					     int color_plane)
{
	...
	intel_de_write_fw(xe, SEL_FETCH_PLANE_POS(pipe, plane->id), val);
	...
	intel_de_write_fw(xe, SEL_FETCH_PLANE_OFFSET(pipe, plane->id),
			  val);

	/* Sizes are 0 based */
	val = (drm_rect_height(clip) - 1) << 16;
	val |= (drm_rect_width(&plane_state->uapi.src) >> 16) - 1;
	intel_de_write_fw(xe, SEL_FETCH_PLANE_SIZE(pipe, plane->id), val);
}
		       ...
		       const struct intel_plane_state *plane_state)
{
	...
	if (intel_fb_is_rc_ccs_cc_modifier(fb->modifier)) {
		intel_de_write_fw(xe, PLANE_CC_VAL(pipe, plane_id, 0),
				  lower_32_bits(plane_state->ccval));
		intel_de_write_fw(xe, PLANE_CC_VAL(pipe, plane_id, 1),
				  upper_32_bits(plane_state->ccval));
	}

	/* FLAT CCS doesn't need to program AUX_DIST */
	if (!HAS_FLAT_CCS(xe) && DISPLAY_VER(xe) < 20)
		intel_de_write_fw(xe, PLANE_AUX_DIST(pipe, plane_id),
				  skl_plane_aux_dist(plane_state, color_plane));

	if (icl_is_hdr_plane(xe, plane_id))
		intel_de_write_fw(xe, PLANE_CUS_CTL(pipe, plane_id),
				  plane_state->cus_ctl);

	intel_de_write_fw(xe, PLANE_COLOR_CTL(pipe, plane_id), plane_color_ctl);

	if (fb->format->is_yuv && icl_is_hdr_plane(xe, plane_id))
					   ...
					   const struct intel_plane_state *plane_state)
{
	...
	if (drm_rect_height(&plane_state->psr2_sel_fetch_area) > 0)
		intel_de_write_fw(xe, SEL_FETCH_PLANE_CTL(pipe, plane->id),
			      ...
			      const struct intel_plane_state *plane_state)
{
	...
	if ((IS_ALDERLAKE_S(xe) || IS_TIGERLAKE(xe)) &&
...
static int skl_plane_check_nv12_rotation(const struct intel_plane_state *plane_state)
{
	...
	if (intel_format_info_is_yuv_semiplanar(fb->format, fb->modifier) &&
		...
		drm_dbg_kms(&xe->drm, "src width must be multiple of 4 for rotated planar YUV\n");
		return -EINVAL;
	}
...
static int skl_plane_max_scale(struct xe_device *xe,
			       const struct drm_framebuffer *fb)
{
	/*
				 ...
				 int *x, int *y, u32 *offset)
{
	...
	*offset = intel_plane_compute_aligned_offset(x, y, plane_state, 0);
	if (drm_WARN_ON(&xe->drm, alignment && !is_power_of_2(alignment)))
		return -EINVAL;
...
static int skl_check_main_surface(struct intel_plane_state *plane_state)
{
	...
	if (DISPLAY_VER(xe) >= 13)
		drm_WARN_ON(&xe->drm, x > 65535 || y > 65535);
	else
		drm_WARN_ON(&xe->drm, x > 8191 || y > 8191);
...
static bool skl_fb_scalable(const struct drm_framebuffer *fb)
{
	...
	case DRM_FORMAT_XBGR16161616F:
	case DRM_FORMAT_ABGR16161616F:
		return DISPLAY_VER(to_xe(fb->dev)) >= 11;
	default:
		return true;
	}
}
...
static void check_protection(struct intel_plane_state *plane_state)
{
	...
	plane_state->decrypt = intel_pxp_key_check(xe->pxp, obj, false) == 0;
			   ...
			   struct intel_plane_state *plane_state)
{
	...
	if (!plane_state->ckey.flags && skl_fb_scalable(fb)) {
		min_scale = 1;
		max_scale = skl_plane_max_scale(xe, fb);
	}
...
static bool skl_plane_has_fbc(struct xe_device *xe,
			      enum intel_fbc_id fbc_id, enum plane_id plane_id)
{
	if ((DISPLAY_RUNTIME_INFO(xe)->fbc_mask & BIT(fbc_id)) == 0)
		return false;

	if (DISPLAY_VER(xe) >= 20)
		return icl_is_hdr_plane(xe, plane_id);
	else
		return plane_id == PLANE_1;
}

static struct intel_fbc *skl_plane_fbc(struct xe_device *xe,
				       enum pipe pipe, enum plane_id plane_id)
{
	enum intel_fbc_id fbc_id = skl_fbc_id_for_pipe(pipe);

	if (skl_plane_has_fbc(xe, fbc_id, plane_id))
		return xe->display.fbc[fbc_id];
	else
		return NULL;
}

static bool skl_plane_has_planar(struct xe_device *xe,
				 enum pipe pipe, enum plane_id plane_id)
{
	...
	if (DISPLAY_VER(xe) == 9 && pipe == PIPE_C)
		return false;
...
static const u32 *skl_get_plane_formats(struct xe_device *xe,
					enum pipe pipe, enum plane_id plane_id,
					int *num_formats)
{
	if (skl_plane_has_planar(xe, pipe, plane_id)) {
...
static const u32 *glk_get_plane_formats(struct xe_device *xe,
					enum pipe pipe, enum plane_id plane_id,
					int *num_formats)
{
	if (skl_plane_has_planar(xe, pipe, plane_id)) {
...
static const u32 *icl_get_plane_formats(struct xe_device *xe,
					enum pipe pipe, enum plane_id plane_id,
					int *num_formats)
{
	if (icl_is_hdr_plane(xe, plane_id)) {
	...
	} else if (icl_is_nv12_y_plane(xe, plane_id)) {
...
skl_plane_enable_flip_done(struct intel_plane *plane)
{
	...
	bdw_enable_pipe_irq(xe, pipe, GEN9_PIPE_PLANE_FLIP_DONE(plane->id));
...
skl_plane_disable_flip_done(struct intel_plane *plane)
{
	...
	spin_lock_irq(&xe->irq_lock);
	bdw_disable_pipe_irq(xe, pipe, GEN9_PIPE_PLANE_FLIP_DONE(plane->id));
	spin_unlock_irq(&xe->irq_lock);
}

static bool skl_plane_has_rc_ccs(struct xe_device *xe,
				 enum pipe pipe, enum plane_id plane_id)
{
	/* Wa_22011186057 */
...
static bool gen12_plane_has_mc_ccs(struct xe_device *xe,
				   enum plane_id plane_id)
{
	if (DISPLAY_VER(xe) < 12)
		return false;
	...
	/* Wa_14010477008 */
	if (IS_DG1(xe) || IS_ROCKETLAKE(xe) ||
		(IS_TIGERLAKE(xe) && IS_DISPLAY_STEP(xe, STEP_A0, STEP_D0)))
		return false;
...
static u8 skl_get_plane_caps(struct xe_device *xe,
			     enum pipe pipe, enum plane_id plane_id)
{
	u8 caps = INTEL_PLANE_CAP_TILING_X;

	if (DISPLAY_VER(xe) < 13 || IS_ALDERLAKE_P(xe))
	...
	if (!IS_ENABLED(I915) && !HAS_FLAT_CCS(xe))
		return caps;

	if (skl_plane_has_rc_ccs(xe, pipe, plane_id)) {
		caps |= INTEL_PLANE_CAP_CCS_RC;
		if (DISPLAY_VER(xe) >= 12)
			caps |= INTEL_PLANE_CAP_CCS_RC_CC;
	}

	if (gen12_plane_has_mc_ccs(xe, plane_id))
		caps |= INTEL_PLANE_CAP_CCS_MC;

	if (DISPLAY_VER(xe) >= 14 && IS_DGFX(xe))
			   ...
			   enum pipe pipe, enum plane_id plane_id)
{
	...
	intel_fbc_add_plane(skl_plane_fbc(xe, pipe, plane_id), plane);
	...
	if (plane_id == PLANE_1) {
		plane->need_async_flip_toggle_wa = IS_DISPLAY_VER(xe, 9, 10);
	...
	if (DISPLAY_VER(xe) >= 11)
		formats = icl_get_plane_formats(xe, pipe,
						plane_id, &num_formats);
	else if (DISPLAY_VER(xe) >= 10)
		formats = glk_get_plane_formats(xe, pipe,
						plane_id, &num_formats);
	else
		formats = skl_get_plane_formats(xe, pipe,
						plane_id, &num_formats);
	...
	modifiers = intel_fb_plane_get_modifiers(xe,
						 skl_get_plane_caps(xe, pipe, plane_id));

	ret = drm_universal_plane_init(&xe->drm, &plane->base,
				       0, plane_funcs,
			     ...
			     struct intel_initial_plane_config *plane_config)
{
	...
	if (!intel_fb) {
		drm_dbg_kms(&xe->drm, "failed to alloc fb\n");
		return;
	}
	...
	val = intel_de_read(xe, PLANE_CTL(pipe, plane_id));
	...
	if (DISPLAY_VER(xe) >= 10) {
		...
		color_ctl = intel_de_read(xe, PLANE_COLOR_CTL(pipe, plane_id));
	...
	if (!xe->display.params.enable_dpt &&
	    intel_fb_modifier_uses_dpt(xe, fb->modifier)) {
		drm_dbg_kms(&xe->drm, "DPT disabled, skipping initial FB\n");
		goto error;
	}
	...
	if (DISPLAY_VER(xe) >= 11 && val & PLANE_CTL_FLIP_HORIZONTAL)
	...
	base = intel_de_read(xe, PLANE_SURF(pipe, plane_id)) & PLANE_SURF_ADDR_MASK;
	plane_config->base = base;

	offset = intel_de_read(xe, PLANE_OFFSET(pipe, plane_id));
	drm_WARN_ON(&xe->drm, offset != 0);

	val = intel_de_read(xe, PLANE_SIZE(pipe, plane_id));
	fb->height = REG_FIELD_GET(PLANE_HEIGHT_MASK, val) + 1;
	fb->width = REG_FIELD_GET(PLANE_WIDTH_MASK, val) + 1;

	val = intel_de_read(xe, PLANE_STRIDE(pipe, plane_id));
				    ...
				    const struct intel_initial_plane_config *plane_config)
{
	...
	intel_de_write(xe, PLANE_SURF(pipe, plane_id), base);
