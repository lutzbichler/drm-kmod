				   ...
				   bool *repeater_present)
{
	...
	ret = intel_dp_hdcp_read_bcaps(&dig_port->dp.aux, xe, &bcaps);
	if (ret)
		return ret;
				...
				u8 *ri_prime)
{
	...
	if (ret != DRM_HDCP_RI_LEN) {
		drm_dbg_kms(&xe->drm, "Read Ri' from DP/AUX failed (%zd)\n",
			    ret);
		return ret >= 0 ? -EIO : ret;
	}
	return 0;
			    ...
			    u8 msg_id, void *buf, size_t size)
{
	...
	while (bytes_to_recv) {
		...
		ret = drm_dp_dpcd_read(aux, offset,
				       (void *)byte, len);
		if (ret < 0) {
			drm_dbg_kms(&xe->drm, "msg_id %d, ret %zd\n",
				    msg_id, ret);
			return ret;
		}
	...
	if (hdcp2_msg_data->msg_read_timeout > 0) {
		...
		if (msg_expired) {
			drm_dbg_kms(&xe->drm, "msg_id %d, entire msg read timeout(mSec): %d\n",
					...
					bool *hdcp2_capable)
{
	...
	*hdcp2_capable = false;
	...
	ret = intel_dp_hdcp_read_bcaps(aux, xe, &bcaps);
	if (ret)
		return ret;
				       ...
				       bool enable)
{
	...
	if (ret)
		drm_err(&xe->drm, "%s HDCP stream select failed (%d)\n",
				    ...
				    bool enable)
{
	...
	if (intel_de_wait(xe, HDCP_STATUS(xe, cpu_transcoder, port),
			  stream_enc_status, enable ? stream_enc_status : 0,
			  HDCP_ENCRYPT_STATUS_CHANGE_TIMEOUT_MS)) {
		drm_err(&xe->drm, "Timed out waiting for transcoder: %s stream encryption %s\n",
				     ...
				     bool enable)
{
	...
	drm_WARN_ON(&xe->drm, enable &&
		    !!(intel_de_read(xe, HDCP2_AUTH_STREAM(xe, cpu_transcoder, port))
	...
	/* Wait for encryption confirmation */
	if (intel_de_wait(xe, HDCP2_STREAM_STATUS(xe, cpu_transcoder, pipe),
		       ...
		       struct intel_connector *intel_connector)
{
	...
	struct xe_device *dev_priv = to_xe(dev);
