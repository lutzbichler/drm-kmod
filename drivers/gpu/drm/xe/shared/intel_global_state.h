...
#define intel_for_each_global_obj(obj, xe) \
	list_for_each_entry(obj, &(xe)->display.global.obj_list, head)
...
void intel_atomic_global_obj_init(struct xe_device *xe,
				  struct intel_global_obj *obj,
				  struct intel_global_state *state,
				  const struct intel_global_state_funcs *funcs);
void intel_atomic_global_obj_cleanup(struct xe_device *xe);
