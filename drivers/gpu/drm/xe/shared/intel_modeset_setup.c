					      ...
					      struct drm_modeset_acquire_ctx *ctx)
{
	...
	for_each_intel_crtc_in_pipe_mask(&xe->drm, temp_crtc,
					 BIT(pipe) |
		...
		drm_WARN_ON(&xe->drm, IS_ERR(temp_crtc_state) || ret);
	}

	xe->display.funcs.display->crtc_disable(to_intel_atomic_state(state), crtc);
...
static void reset_encoder_connector_state(struct intel_encoder *encoder)
{
	...
	struct intel_pmdemand_state *pmdemand_state =
		to_intel_pmdemand_state(xe->display.pmdemand.obj.state);
	...
	for_each_intel_connector_iter(connector, &conn_iter) {
		...
		intel_pmdemand_update_phys_mask(xe, encoder,
...
static void reset_crtc_encoder_state(struct intel_crtc *crtc)
{
	...
	for_each_encoder_on_crtc(&xe->drm, &crtc->base, encoder) {
...
static void intel_crtc_disable_noatomic_complete(struct intel_crtc *crtc)
{
	...
	struct intel_bw_state *bw_state =
		to_intel_bw_state(xe->display.bw.obj.state);
	struct intel_cdclk_state *cdclk_state =
		to_intel_cdclk_state(xe->display.cdclk.obj.state);
	...
	intel_display_power_put_all_in_set(xe, &crtc->enabled_power_domains);
	...
	intel_pmdemand_update_port_clock(xe, pmdemand_state, pipe, 0);
}
 ...
 */
static u8 get_transcoder_pipes(struct xe_device *xe,
			       u8 transcoder_mask)
{
	struct intel_crtc *temp_crtc;
	u8 pipes = 0;

	for_each_intel_crtc(&xe->drm, temp_crtc) {
		struct intel_crtc_state *temp_crtc_state =
			       ...
			       u8 *master_pipe_mask, u8 *slave_pipes_mask)
{
	...
	*master_pipe_mask = get_transcoder_pipes(xe, BIT(master_transcoder));
	drm_WARN_ON(&xe->drm, !is_power_of_2(*master_pipe_mask));

	master_crtc = intel_crtc_for_pipe(xe, ffs(*master_pipe_mask) - 1);
	master_crtc_state = to_intel_crtc_state(master_crtc->base.state);
	*slave_pipes_mask = get_transcoder_pipes(xe, master_crtc_state->sync_mode_slaves_mask);
}

static u8 get_joiner_secondary_pipes(struct xe_device *xe, u8 primary_pipes_mask)
{
	struct intel_crtc *primary_crtc;
	u8 pipes = 0;

	for_each_intel_crtc_in_pipe_mask(&xe->drm, primary_crtc, primary_pipes_mask) {
					...
					struct drm_modeset_acquire_ctx *ctx)
{
	...
	joiner_secondaries_mask = get_joiner_secondary_pipes(xe,
	...
	for_each_intel_crtc_in_pipe_mask(&xe->drm, temp_crtc, joiner_secondaries_mask)
		intel_crtc_disable_noatomic_begin(temp_crtc, ctx);

	for_each_intel_crtc_in_pipe_mask(&xe->drm, temp_crtc, portsync_slaves_mask)
		intel_crtc_disable_noatomic_begin(temp_crtc, ctx);

	for_each_intel_crtc_in_pipe_mask(&xe->drm, temp_crtc, portsync_master_mask)
		intel_crtc_disable_noatomic_begin(temp_crtc, ctx);
...
static void intel_modeset_update_connector_atomic_state(struct xe_device *xe)
{
	struct intel_connector *connector;
	struct drm_connector_list_iter conn_iter;
...
static void intel_crtc_copy_hw_to_uapi_state(struct intel_crtc_state *crtc_state)
{
	...
	if (DISPLAY_INFO(xe)->color.degamma_lut_size) {
		/* assume 1:1 mapping */
		 ...
		 * in the uapi.
		 */
		drm_WARN_ON(&xe->drm, crtc_state->post_csc_lut &&
			    crtc_state->pre_csc_lut);
...
static void
intel_sanitize_plane_mapping(struct xe_device *xe)
{
	struct intel_crtc *crtc;
	...
	for_each_intel_crtc(&xe->drm, crtc) {
		...
		plane_crtc = intel_crtc_for_pipe(xe, pipe);
...
static void intel_sanitize_fifo_underrun_reporting(const struct intel_crtc_state *crtc_state)
{
	 ...
	 */
	intel_init_fifo_underrun_reporting(xe, crtc,
...
static void intel_sanitize_all_crtcs(struct xe_device *xe,
				     struct drm_modeset_acquire_ctx *ctx)
{
	struct intel_crtc *crtc;
...
static void readout_plane_state(struct xe_device *xe)
{
	struct intel_plane *plane;
	struct intel_crtc *crtc;
...
static void intel_modeset_readout_hw_state(struct xe_device *xe)
{
	struct intel_cdclk_state *cdclk_state =
		to_intel_cdclk_state(xe->display.cdclk.obj.state);
	struct intel_dbuf_state *dbuf_state =
	...
	intel_dpll_readout_hw_state(xe);
	...
	for_each_intel_crtc(&xe->drm, crtc) {
		...
		for_each_intel_plane_on_crtc(&xe->drm, crtc, plane) {
			...
			if (plane_state->uapi.visible && plane->min_cdclk) {
				if (crtc_state->double_wide || DISPLAY_VER(xe) >= 10)
		...
		if (crtc_state->hw.active) {
			...
			if (drm_WARN_ON(&xe->drm, min_cdclk < 0))
				min_cdclk = 0;
		}
		...
		intel_pmdemand_update_port_clock(xe, pmdemand_state, pipe,
						 crtc_state->port_clock);

		intel_bw_crtc_update(bw_state, crtc_state);
	}

	intel_pmdemand_init_pmdemand_params(xe, pmdemand_state);
}

static void
get_encoder_power_domains(struct xe_device *xe)
{
	struct intel_encoder *encoder;
...
static void intel_early_display_was(struct xe_device *xe)
{
	/*
	...
	if (IS_DISPLAY_VER(xe, 10, 12))
		intel_de_rmw(xe, GEN9_CLKGATE_DIS_0, 0, DARBF_GATING_DIS);
	 ...
	 */
	if (IS_HASWELL(xe))
		intel_de_rmw(xe, CHICKEN_PAR1_1, 0, FORCE_ARB_IDLE_PLANES);
				  ...
				  struct drm_modeset_acquire_ctx *ctx)
{
	...
	wakeref = intel_display_power_get(xe, POWER_DOMAIN_INIT);

	intel_early_display_was(xe);
	intel_modeset_readout_hw_state(xe);

	/* HW state is read out, now we need to sanitize this mess. */
	get_encoder_power_domains(xe);

	intel_pch_sanitize(xe);
	...
	for_each_intel_crtc(&xe->drm, crtc) {
		...
		if (crtc_state->hw.active) {
			intel_dmc_enable_pipe(xe, crtc->pipe);
			intel_crtc_vblank_on(crtc_state);
		}
	}

	intel_fbc_sanitize(&xe->display);

	intel_sanitize_plane_mapping(xe);
	 ...
	 */
	intel_modeset_update_connector_atomic_state(xe);

	intel_sanitize_all_crtcs(xe, ctx);

	intel_dpll_sanitize_state(xe);

	intel_wm_get_hw_state(xe);
	...
	for_each_intel_crtc(&xe->drm, crtc) {
		...
		if (drm_WARN_ON(&xe->drm, !bitmap_empty(put_domains.bits, POWER_DOMAIN_NUM)))
			intel_modeset_put_crtc_power_domains(crtc, &put_domains);
	}

	intel_display_power_put(xe, POWER_DOMAIN_INIT, wakeref);

	intel_power_domains_sanitize_state(xe);
}
