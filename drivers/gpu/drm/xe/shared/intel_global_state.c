				  ...
				  const struct intel_global_state_funcs *funcs)
{
	...
	obj->state = state;
	obj->funcs = funcs;
	list_add_tail(&obj->head, &xe->display.global.obj_list);
}
...
void intel_atomic_global_obj_cleanup(struct xe_device *xe)
{
	...
	list_for_each_entry_safe(obj, next, &xe->display.global.obj_list, head) {
		list_del(&obj->head);

		drm_WARN_ON(&xe->drm, kref_read(&obj->state->ref) != 1);
		intel_atomic_global_state_put(obj->state);
	}
}

static void assert_global_state_write_locked(struct xe_device *xe)
{
...
static void assert_global_state_read_locked(struct intel_atomic_state *state)
{
	...
	drm_WARN(&xe->drm, 1, "Global state not read locked\n");
}
				  ...
				  struct intel_global_obj *obj)
{
	...
	drm_dbg_atomic(&xe->drm, "Added new global object %p state %p to %p\n",
		       obj, obj_state, state);
...
void intel_atomic_swap_global_state(struct intel_atomic_state *state)
{
	...
	for_each_oldnew_global_obj_in_state(state, obj, old_obj_state,
					    new_obj_state, i) {
		drm_WARN_ON(&xe->drm, obj->state != old_obj_state);
		...
		assert_global_state_write_locked(xe);
...
intel_atomic_global_state_wait_for_dependencies(struct intel_atomic_state *state)
{
	...
	for_each_old_global_obj_in_state(state, obj, old_obj_state, i) {
		...
		if (ret == 0) {
			drm_err(&xe->drm, "global state timed out\n");
			return -ETIMEDOUT;
		}
	}
