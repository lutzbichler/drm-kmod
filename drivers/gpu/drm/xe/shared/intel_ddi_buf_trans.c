...
void intel_ddi_buf_trans_init(struct intel_encoder *encoder)
{
	...
	if (DISPLAY_VER(xe) >= 14) {
	...
	} else if (IS_COMETLAKE_ULX(xe) || IS_COFFEELAKE_ULX(xe) || IS_KABYLAKE_ULX(xe)) {
		encoder->get_buf_trans = kbl_y_get_buf_trans;
	} else if (IS_COMETLAKE_ULT(xe) || IS_COFFEELAKE_ULT(xe) || IS_KABYLAKE_ULT(xe)) {
		encoder->get_buf_trans = kbl_u_get_buf_trans;
	} else if (IS_COMETLAKE(xe) || IS_COFFEELAKE(xe) || IS_KABYLAKE(xe)) {
		encoder->get_buf_trans = kbl_get_buf_trans;
	} else if (IS_SKYLAKE_ULX(xe)) {
		encoder->get_buf_trans = skl_y_get_buf_trans;
	} else if (IS_SKYLAKE_ULT(xe)) {
		encoder->get_buf_trans = skl_u_get_buf_trans;
	} else if (IS_SKYLAKE(xe)) {
		...
		MISSING_CASE(INTEL_INFO(xe)->platform);
	}
}
